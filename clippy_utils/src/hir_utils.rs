...
use crate::source::{IntoSpanData, SpanExt, walk_span_to_context};
...
fn eq_span_tokens(
    cx: &LateContext<'_>,
    left: impl IntoSpanData,
    right: impl IntoSpanData,
    ...
    fn f(cx: &LateContext<'_>, left: Range<BytePos>, right: Range<BytePos>, pred: impl Fn(TokenKind) -> bool) -> bool {
        if let Some(lsrc) = left.get_source_range(cx)
            && let Some(lsrc) = lsrc.src_text()
            && let Some(rsrc) = right.get_source_range(cx)
            && let Some(rsrc) = rsrc.src_text()
        {
