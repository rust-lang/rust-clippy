...
impl<'tcx> ConstEvalCtxt<'tcx> {
    ...
    fn fetch_path(&self, qpath: &QPath<'_>, id: HirId) -> Option<ConstValue> {
        ...
        let did = match *qpath {
            ...
            _ if let Res::Def(DefKind::Const | DefKind::AssocConst, did) = typeck.qpath_res(qpath, id)
                && !tcx.is_direct_const(did) =>
            {
        ...
        let args = typeck.node_args(id);

        if !args.is_empty() {
            let owner_def_id = typeck.hir_owner.def_id.to_def_id();
            let identity_args = ty::GenericArgs::identity_for_item(*tcx, owner_def_id);
            // Don't try to fully evaluate consts inside code whose bounds can't be satisfied.
            if tcx.instantiate_and_check_impossible_clauses((owner_def_id, identity_args)) {
                return None;
            }
        }

        tcx.const_eval_resolve(*typing_env, mir::UnevaluatedConst::new(did, args), qpath.span())
            .ok()
    }
