...
use rustc_middle::ty::{
    ...
    GenericParamDefKind, IntTy, ParamTy, Region, RegionKind, TraitRef, Ty, TyCtxt, TypeSuperVisitable, TypeVisitable,
...
use crate::{is_diag_trait_item, path_res};
...
pub fn is_to_string_on_string_like<'a>(cx: &LateContext<'_>, call_expr: &'a Expr<'a>, method_def_id: DefId) -> bool {
    is_to_string(cx, call_expr, method_def_id) && is_on_string_like(cx, call_expr)
}

/// Returns true if the named method is `ToString::to_string`.
pub fn is_to_string(cx: &LateContext<'_>, call_expr: &Expr<'_>, method_def_id: DefId) -> bool {
    let hir::ExprKind::MethodCall(method_name, _, _, _) = call_expr.kind else {
        return false;
    };

    if method_name.ident.name != sym::to_string || !is_diag_trait_item(cx, method_def_id, sym::ToString) {
        return false;
    }
