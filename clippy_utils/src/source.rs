...
fn dbg_check_range(sm: &SourceMap, text: &SourceText, old: Option<FileRange>, new: FileRange) {
    ...
    if text.get(new.clone().into_slice_idx()).is_none() {
        ...
        let files_end = sm
            .files()
            .last()
            .map_or(0, |f| f.start_pos.0 + f.normalized_source_len.0);
        let mut print_loc = |label: &str, pos: BytePos| {
            if pos.0 <= files_end {
                ...
                if let Some(text) = SourceText::for_local_file(sfp.sf.clone())
                    && text.get(sfp.pos.to_usize()..).is_some()
                {
                    ...
                    // No valid column position , but still get a line number.
        ...
        print_loc("start", new_start);
        print_loc("end", new_end);

        if let Some(old) = old
            && let Some(old_text) = text.get(old.clone().into_slice_idx())
        {
            // If this is for debugging a range transformation also include the previous range.
            // Don't bother reporting if the previous range is invalid, we aren't debug checking
            // that one.
            ...
            // Also include the text of the range with each line indented.
            msg.extend(old_text.split('\n').flat_map(|x| ["\n  ", x]));
        } else {
            // Display the expected file of the span in case the range bounds are located
            // in a different file.
            let _ = write!(msg, "\n  expected file: {}", file.name.prefer_local_unconditionally());
        }
...
pub trait FileRangeExt: Sized + FileIndex {
    ...
    #[must_use]
    #[cfg_attr(debug_assertions, track_caller)]
    fn get_range_between(self, scx: &SpanEditCx<'_>, other: impl ToSpanData) -> Option<FileRange> {
...
impl FileRangeExt for FileRange {
    #[inline]
    #[cfg_attr(debug_assertions, track_caller)]
    fn get_range_between(self, scx: &SpanEditCx<'_>, sp: impl ToSpanData) -> Option<FileRange> {
        #[inline]
        #[cfg_attr(debug_assertions, track_caller)]
