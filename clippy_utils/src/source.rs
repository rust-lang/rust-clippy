...
impl<'sm> SourceFileRange<'sm> {
    ...
    /// Gets the range relative to the source map.
    #[inline]
    #[must_use]
    ...
    pub fn add_leading_whitespace(&mut self) -> Option<&mut Self> {
        self.edit_range(|sf, src, range| {
            let text_before = src.get(..range.start)?.trim_end();
            let lines = sf.lines();
            ...
            if let Some(search_line_end) = lines.get(post_search_line)
                ...
                && search_line_end.to_usize() <= range.start
                ...
                && tokenize(src.get(range.end..next_start)?).any(|t| !matches!(t.kind, TokenKind::Whitespace))
            {
                ...
                Some(text_before.len()..range.end)
            }
        })
    }
