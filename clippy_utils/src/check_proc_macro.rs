...
use rustc_ast::ast::{
    AttrKind, Attribute, BindingMode, GenericArgs, IntTy, LitIntType, LitKind, RangeLimits, StrStyle, StructExpr,
...
fn token_lit_search_pat(lit: &ast::token::Lit) -> (Pat, Pat) {
    ...
    match lit.kind {
        ...
        LitKind::Integer | LitKind::Float => (Pat::Sym(lit.symbol), Pat::Sym(lit.suffix.unwrap_or(lit.symbol))),
...
fn ast_expr_search_pat(e: &ast::Expr) -> (Pat, Pat) {
    ...
    fn inner(e: &ast::Expr, outer_span: Span) -> (Pat, Pat) {
        ...
        match &e.kind {
            ...
            // `move(expr)`
            ExprKind::Move(_, _) => (Pat::Str("move"), Pat::Str("")),
            ExprKind::OffsetOf(..)
            // Syntax unstable
