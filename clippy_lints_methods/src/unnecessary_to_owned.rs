...
use crate::utils::clone_or_copy_needed;
...
use clippy_utils::visitors::{find_all_ret_expressions, for_each_expr_without_closures};
use clippy_utils::{
    can_mut_borrow_both, fn_def_id, get_parent_expr, is_diag_item_method, is_diag_trait_item, is_expr_temporary_value,
    path_to_local, peel_middle_ty_refs, return_ty, sym,
};
use core::ops::ControlFlow;
use rustc_errors::Applicability;
use rustc_hir::def::{DefKind, Res};
use rustc_hir::def_id::DefId;
use rustc_hir::{BindingMode, BorrowKind, Expr, ExprKind, ItemKind, LangItem, Node, PatKind};
...
pub fn check<'tcx>(
        ...
        && args.is_empty()
    {
        if is_cloned_or_copied(cx, method_name, method_def_id) {
            check_iter_cloned(cx, expr, method_name, receiver);
...
fn check_iter_cloned(cx: &LateContext<'_>, expr: &Expr<'_>, method_name: Symbol, receiver: &Expr<'_>) -> bool {
    if let Some(parent) = get_parent_expr(cx, expr)
        && let Some(callee_def_id) = fn_def_id(cx, parent)
        && is_into_iter(cx, callee_def_id)
    {
...
fn is_into_iter(cx: &LateContext<'_>, callee_def_id: DefId) -> bool {
