...
use clippy_utils::source::{IntoSpan as _, SpanExt as _, snippet_indent};
...
declare_tool_lint! {
    ...
    ///
    /// ### Known problems
    /// FN if `impl_lint_pass!` is used in a qualified way, e.g. as `rustc_lint::impl_lint_pass!`
...
impl<'tcx> LateLintPass<'tcx> for NeedlessImplLintPass {
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx Item<'tcx>) {
        ...
        if let ItemKind::Impl(impl_) = item.kind
            ...
            && let &Res::Def(DefKind::Struct, lint_pass_struct) = impl_.self_ty.basic_res()
            // we'll need a `LocalDefId` later. This should always hold, as `impl_lint_pass!`
            // doesn't support generic types.
            && let Some(lint_pass_struct) = lint_pass_struct.as_local()
            // make sure this all happens inside `impl_lint_pass!`
            && let Some(impl_lint_pass_span) = is_expn_of(item.span, sym::impl_lint_pass)
            ...
            && cx.tcx.adt_def(lint_pass_struct).non_enum_variant().fields.is_empty()
        {
            span_lint_and_then(
                cx,
                ...
                |diag| {
                    const HELP_MSG: &str =
                        "remove the struct declaration, and replace `impl_lint_pass!` with `declare_lint_pass!`";
                    ...
                    // Cut out the `impl` from `impl_lint_pass!`
                    if let Some(impl_span) = impl_lint_pass_span.map_range(cx, |_, src, range| {
                        src.get(range.clone())?
                            .starts_with("impl_lint_pass")
                            .then_some(range.start..range.start + 4)
                    }) {
                        ...
                        let sugg = vec![
                            ...
                            // replace `impl` with `declare`
                            (impl_span.into_span(), "declare".to_string()),
                        ];
