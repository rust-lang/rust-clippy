...
pub fn register_metadata_collector(store: &mut LintStore) {
    store.register_lints(LINTS);
    store.register_lints(&[metadata_collector::METADATA_COLLECTOR]);
    store.register_late_pass(|_| Box::new(metadata_collector::MetadataCollector::new()));
}
