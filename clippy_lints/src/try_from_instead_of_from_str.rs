...
declare_clippy_lint! {
    ...
    /// ### Why is this bad?
    ///
    /// When implementing `FromStr` on a type, the intent of the conversion is more explicit. It
    /// also allows to call `.parse()` on a string to get it converted into the type as it will
    /// call the `FromStr` implementation under the hood.
    ///
    /// ### Example
    ///
    /// ```no_run
