...
impl<'tcx> LateLintPass<'tcx> for DurationToFloatPrecisionLoss {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, root: &'tcx Expr<'_>) {
        if in_external_macro(cx.sess(), root.span) {
            // We should ignore macro from a foreign crate.
            return;
        }

        let site = match root.kind {
            ExprKind::Cast(duration_expr, _) => {
                let Some(lint_site) = check_cast(cx, root, duration_expr) else {
            ...
            ExprKind::Binary(op, lhs, rhs) => {
                if let ExprKind::Cast(duration_expr, _) = lhs.kind {
                    let Some(mut lint_site) = check_cast(cx, root, duration_expr) else {
                        // does not match criteria - cast didn't have right contents
                        return;
                    };
            ...
            _ => {
                // does not match criteria - root expression was not a cast or a binary op
                return;
            },
        };
        ...
        if self.should_emit_lint(&site) {
            site.emit_lint(cx);
            self.lint_applications.push(site.root_span);
        }
    }

    fn check_expr_post(&mut self, _: &LateContext<'tcx>, root: &'tcx Expr<'tcx>) {
        // clean up the stack
        if self
            .lint_applications
            .last()
            .is_some_and(|root_span| root_span == &root.span)
        {
...
fn check_cast<'tcx>(
    ...
    if let ExprKind::MethodCall(method_path, method_receiver_expr, [], _) = duration_expr.kind {
        ...
        if is_type_diagnostic_item(cx, method_receiver_ty.peel_refs(), sym::Duration) {
            let cast_expr_ty = cx.typeck_results().expr_ty(root);
...
struct LintApplicableSite {
    ...
    // When the adjustment is missing from the site, set this to (`One`, whatever)
...
impl LintApplicableSite {
    ...
    fn emit_lint(&self, cx: &LateContext<'_>) {
        ...
        if !self.root_span.eq_ctxt(self.duration_expr_span) {
