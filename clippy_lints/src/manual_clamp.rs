...
use clippy_utils::{eq_expr_value, is_in_const_context, is_integer_const, peel_blocks, peel_blocks_with_stmt, sym};
...
fn try_normalize_unsigned_eq_to_ord<'tcx>(
    ...
    let input = if is_integer_const(cx, bin.right, 0) {
        bin.left
    } else if is_integer_const(cx, bin.left, 0) {
        bin.right
    } else {
        return None;
    };

    if is_integer_const(cx, result_expr, 1) {
        Some(BinaryOp {
            op: BinOpKind::Lt,
            left: input,
            right: result_expr,
