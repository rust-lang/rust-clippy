...
impl<'tcx> UseScanner<'tcx> {
    fn examine_use(&mut self, def_id: DefId) {
        if let Some(node) = self.tcx.hir().get_if_local(def_id) {
            match node {
                // TODO: TraitItem, ImplItem, Field
                hir::Node::ForeignItem(hir::ForeignItem { hir_id, .. }) | hir::Node::Item(hir::Item { hir_id, .. }) => {
                    if let Some(watch_item) = self.watched_item_map.get_mut(&hir_id) {
                        if self
                            .current_module_path
                            .iter()
                            ...
                            // `pub(crate)`.
                            .any(|mod_| *mod_ == watch_item.enclosing_module)
                        {
                            watch_item.status.observe(LocalReference);
                        } else {
                            watch_item.status.observe(CrateReference);
                        }
                    }
                },
                _ => {},
...
impl<'tcx> LateLintPass<'tcx> for UnneededPubCrate {
    ...
    // TODO: methods, struct fields, const/static, alias, modules. what else can have visibility?
    ...
    fn check_crate_post(&mut self, cx: &LateContext<'tcx>, crate_: &'tcx hir::Crate<'tcx>) {
        ...
        for (_, watched_item) in use_scanner.watched_item_map {
