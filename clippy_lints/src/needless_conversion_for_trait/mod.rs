...
const WATCHED_INHERENT_FUNCTIONS: &[ConfPath<SymPath<'static>, false>] = conf_paths!(
    &[sym::slice, sym::as_mut_slice],
    &[sym::slice, sym::as_slice],
    ...
    &[sym::std, sym::ffi, sym::os_str, sym::OsStr, sym::as_os_str],
    ...
    &[sym::std, sym::path, sym::Path, sym::as_path],
...
const IGNORED_INHERENT_FUNCTIONS: &[ConfPath<SymPath<'static>, false>] = conf_paths!(
    ...
    &[sym::str, sym::to_casefold_unnormalized],
    &[sym::str, sym::to_lowercase],
    &[sym::str, sym::to_uppercase],
    &[sym::str, sym::word_to_titlecase],
    ...
    &[sym::alloc, sym::vec, sym::Vec, sym::const_make_global],
            ...
            && let Param(param_ty) = input.kind()
        {
                ...
                loop {
                            ...
                            || is_on_string_like(cx, expr))
                    {
                        if !self
                            ...
                            // cannot be added to `WATCHED_INHERENT_FUNCTIONS`.
            ...
            if let Some((inner_arg, refs_prefix)) = strip_unnecessary_conversions(expr, ancestor_mutabilities) {
                    ...
                    && let Some(span) = maybe_arg.span.parent_callsite()
                {
                    ...
                    let (snippet, _) =
                        snippet_with_context(cx, inner_arg.span, expr.span.ctxt(), "..", &mut applicability);
                    span_lint_and_sugg(
                        cx,
                        NEEDLESS_CONVERSION_FOR_TRAIT,
                        maybe_arg.span,
...
fn peel_boxes<'tcx>(cx: &LateContext<'tcx>, mut expr: &'tcx Expr<'tcx>) -> &'tcx Expr<'tcx> {
    loop {
        if let ExprKind::Call(callee, [_uninit_box, inner_arg]) = expr.kind
            ...
            && paths::WRITE_BOX_VIA_MOVE.matches(cx, *callee_def_id)
        {
            expr = inner_arg;
            continue;
        }
