...
use rustc_hir::{
    Body, ConstArgKind, ExprKind, FnDecl, FnRetTy, GenericParamKind, HirId, Lifetime, LifetimeKind, LifetimeParamKind,
    LifetimeSource, LifetimeSyntax, MissingLifetimeKind, Mutability, Node, Param, Pat, PatKind, QPath, Ty, TyKind,
};
...
struct ArgTypeData {
    ...
    tyname: Option<Symbol>,         // name of type, if any
...
impl<'tcx> LateLintPass<'tcx> for InterchangeableParams {
    ...
    fn check_fn(
        ...
        if let FnKind::ItemFn(ident, ..) = fnkind {
            if ident.name.as_str() != "from_keys" {
                return;
            }
        }
...
fn process_argtypes(
    ...
    match argtype.kind {
        ...
        TyKind::Ref(lifetime, mutty) => {
            ...
            dbg!(argtype);
            match mutty.ty.kind {
                TyKind::Path(qpath) => Some(ArgTypeData {
                    argtype: ArgTypeEnum::Ref,
                    tyname: Some(qpath_to_name(cx, qpath, mutty.ty.hir_id))?,
                    is_unsafe,
                    is_mut: matches!(mutty.mutbl, Mutability::Mut),
                    ..Default::default()
                }),
                TyKind::Slice(ty) => {
                    if let TyKind::Path(qpath) = ty.kind {
                        Some(ArgTypeData {
                            argtype: ArgTypeEnum::Ref,
                            tyname: qpath_to_name(cx, qpath, ty.hir_id),
                            is_unsafe,
                            is_mut: matches!(mutty.mutbl, Mutability::Mut),
                            is_slice: true,
                            ..Default::default()
                ...
                TyKind::Array(thistype, size) => {
                    dbg!(&size);
                    dbg!(&thistype);
                    match size.kind {
                        ConstArgKind::Anon(body) => {
                            let body = cx.tcx.hir_body(body.body);
                            dbg!(body);
                            match body.value.kind {
                                ExprKind::Lit(_) => {
                                    let ecx: ConstEvalCtxt<'_> = ConstEvalCtxt::new(cx);
                                    let ctxt = body.value.span.ctxt();
                                    if let Some(Constant::Int(i)) = ecx.eval_local(body.value, ctxt) {
                                        dbg!(&i);
                                        Some(ArgTypeData {
                                            argtype: ArgTypeEnum::Ref,
                                            is_mut: mutty.mutbl.is_mut(),
                                            lifetime: Some(*lifetime),
                                            arraytype: Some(ArrayType::Number(i)),
                                            ..Default::default()
                                        })
                                    } else {
                                        None
                                ...
                                ExprKind::Path(qpath) => {
                                    let nameopt = qpath_to_name(cx, qpath, size.hir_id);
                                    let name = nameopt?;
                                    dbg!(&name);
                                    Some(ArgTypeData {
                                        argtype: ArgTypeEnum::Ref,
                                        is_mut: mutty.mutbl.is_mut(),
                                        lifetime: Some(*lifetime),
                                        arraytype: Some(ArrayType::Symbol(name)),
                                        ..Default::default()
                                    })
                                },
                                _ => None,
...
fn build_type_text(
    ...
    let ptypename = match typeatd.tyname {
        Some(ref tyname) => tyname.as_str().to_string(),
        None => String::new(),
    };
