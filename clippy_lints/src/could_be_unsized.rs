...
use rustc_hir::{AssocItemKind, GenericParam, Item, ItemKind, Node};
use rustc_lint::{LateContext, LateLintPass};
use rustc_middle::ty::{
    self, AliasKind, AssocKind, Clause, ClauseKind, GenericArg, GenericArgKind, ParamTy, TermKind, Ty, TyCtxt,
    TypeAndMut, TypeFlags, TypeVisitableExt,
};
    ...
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx Item<'_>) {
        match &item.kind {
            ...
            ItemKind::Trait(_, _, generics, _, items) => {
                ...
                for assoc_item in items.iter().filter_map(|i| match &i.kind {
                    AssocItemKind::Type => Some(cx.tcx.hir().trait_item(i.id)),
                    _ => None,
                }) {
                    resolve_deps_for(
                        cx.tcx,
                    ...
                    match params_sizedness.split_last() {
                        Some((item_sizedness, params_sizedness)) => {
...
fn init_sizedness_for_assoc_ty(tcx: TyCtxt<'_>, id: DefId) -> Box<[Cell<Sizedness>]> {
    ...
    vec![Cell::new(Sizedness::Unsized); tcx.generics_of(id).params.len() + 1].into_boxed_slice()
}

fn init_sizedness_for_item(tcx: TyCtxt<'_>, id: DefId) -> Box<[Cell<Sizedness>]> {
    debug_assert!(tcx.def_kind(id) != DefKind::AssocTy);
    vec![Cell::new(Sizedness::Unsized); tcx.generics_of(id).params.len()].into_boxed_slice()
}
...
impl<'tcx> HirParams<'tcx> {
    ...
    fn get_params(tcx: TyCtxt<'tcx>, id: DefId) -> &'tcx [GenericParam<'tcx>] {
        if let Some(Node::Item(item)) = tcx.hir().get_if_local(id)
...
impl<'tcx> AssocHirParams<'tcx> {
    ...
    fn from_def_id(tcx: TyCtxt<'tcx>, id: DefId) -> Self {
        ...
        if let Some(Node::TraitItem(item)) = tcx.hir().get_if_local(id) {
            Self {
...
fn assoc_container_is_trait(tcx: TyCtxt<'_>, assoc_id: DefId, maybe_trait_id: DefId) -> bool {
    if tcx.associated_item(assoc_id).container == ty::TraitContainer {
...
fn add_dep_to_ty<'tcx>(
    ...
    match *ty.kind() {
        ...
        ty::Alias(AliasKind::Projection, ty) => {
...
fn add_deps_from_ty<'tcx>(
    ...
    if !ty.has_type_flags(TypeFlags::HAS_TY_PARAM | TypeFlags::HAS_PROJECTION) {
        return;
    }
    ...
    match *ty.kind() {
        ...
        ty::RawPtr(TypeAndMut { ty, .. }) | ty::Ref(_, ty, _) => {
    ...
    for (pred, pred_span) in predicates {
        match pred.kind().skip_binder() {
            ...
            ClauseKind::Projection(pred) => {
                if let TermKind::Ty(constraint_ty) = pred.term.unpack() {
                    let (param, _) = params.assoc_ty_param(tcx, pred.projection_ty.def_id);
                    let dep = Dependency::new_assoc_item(pred.projection_ty.trait_def_id(tcx), param);
