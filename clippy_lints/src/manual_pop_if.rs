...
/// Returns the span of the first direct use of `collection_id` in the predicate, including nested
/// closures, or `None` if there is no such use. Used to highlight where the predicate borrows the
/// collection when suggesting `pop_if` would introduce a conflicting mutable borrow.
...
/// Checks whether the predicate, including nested closures, uses any local other than `binding_id`,
/// the closure parameter or `if let` binding.
///
/// Conservatively treats all other locals as possible aliases borrowing the collection, so the
/// caller can suppress suggestions that would introduce a conflicting mutable borrow. This also
/// includes locals declared inside the predicate; their scopes and aliasing are not analyzed.
fn predicate_uses_outer_local<'tcx>(cx: &LateContext<'tcx>, predicate: &'tcx Expr<'tcx>, binding_id: HirId) -> bool {
    for_each_expr(cx.tcx, predicate, |expr| {
        if expr.res_local_id().is_some_and(|id| id != binding_id) {
            ControlFlow::Break(())
        } else {
            ControlFlow::Continue(())
        }
...
impl ManualPopIfPattern<'_> {
    fn emit_lint(self, cx: &LateContext<'_>) {
        ...
        let lint_span = self
            .collection_use_span
            .map_or_else(|| self.spans.clone(), MultiSpan::from_span);
            ...
            |diag| {
                ...
                match self.suggestion_kind {
                    ...
                    SuggestionKind::Unavailable => {
                        if self.collection_use_span.is_some() {
                            diag.help(format!(
...
impl<'tcx> LateLintPass<'tcx> for ManualPopIf {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
        ...
        for kind in [
            ...
            {
                if predicate_uses_outer_local(cx, pattern.predicate, pattern.binding_id) {
                    if let Some(id) = pattern.collection_expr.res_local_id()
                        && is_local_used(cx, pattern.predicate, id)
                    {
                        pattern.collection_use_span = collection_use_span(cx, pattern.predicate, id);
                    }
                    pattern.suggestion_kind = SuggestionKind::Unavailable;
                } else if pattern.collection_expr.res_local_id().is_none() {
                    pattern.suggestion_kind = SuggestionKind::Unavailable;
                }
