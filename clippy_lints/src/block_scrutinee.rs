            ...
            && let Some(inner_expr) = block.expr
        {
            ...
            span_lint_and_then(cx, BLOCK_SCRUTINEE, scrutinee.span, main_msg, |diag| {
                ...
                let suggestion_msg = format!(
                    "if you intended to drop temporaries after the surrounding {keyword}, remove the block to clearly communicate this intent"
                );
