...
impl<'tcx> LateLintPass<'tcx> for BlockScrutinee {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        let (scrutinee, keyword, outer_span) = match expr.kind {
            ExprKind::Match(scrutinee, _, _) => (scrutinee, "`match`", expr.span),
            ExprKind::Let(let_expr) => {
                let mut kw = "`if let`";
                let mut outer = expr.span;
                ...
                (let_expr.init, kw, outer)
            },
            _ => return,
        };

        if scrutinee.span.from_expansion() || outer_span.from_expansion() {
            return;
        }
            ...
            && let Some(inner_expr) = block.expr
        {
            ...
            span_lint_and_then(cx, BLOCK_SCRUTINEE, scrutinee.span, main_msg, |diag| {
                diag.note("in Rust 2024, temporaries are dropped earlier");
                diag.help("to drop temporaries early, update your `Cargo.toml` to the 2024 edition");

                let suggestion_msg =
                    format!("if you intend to drop temporaries after the surrounding {keyword}, remove the block");
                ...
                diag.multipart_suggestion(
                    "if you intend to drop temporaries early, move them to a separate local binding",
                    vec![
