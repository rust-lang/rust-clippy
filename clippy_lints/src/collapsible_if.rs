...
use clippy_utils::source::{FileRangeExt as _, SpanExt as _, StrExt as _, snippet, snippet_block_with_applicability};
use clippy_utils::{can_use_if_let_chains, span_contains_non_whitespace, sym, text_contains_cfg, tokenize_with_text};
use rustc_ast::{BinOpKind, MetaItemInner};
use rustc_errors::Applicability;
use rustc_hir::{Block, Expr, ExprKind, StmtKind};
use rustc_lexer::TokenKind;
use rustc_lint::{LateContext, LateLintPass, Level, impl_lint_pass};
use rustc_span::{BytePos, RelativeBytePos, Span, Symbol};
        ...
        if let Some(inner) = expr_block(then)
            ...
            && let Some([then_open_bracket, then_closing_bracket]) = then.span.map_split_range(cx, |scx, range| {
                range
                    .map_split_range_text(scx, |src| src.get_prefix_suffix('{', '}'))
                    .filter(|[_, end]| {
                        scx.get_text(scx.span_to_file_range(inner.span).end..end.start)
                            .is_some_and(|x| !text_contains_cfg(x))
                    })?
                    .try_map(|r| r.with_leading_whitespace(scx))
            })
...
pub(super) fn peel_parens(cx: &LateContext<'_>, span: Span) -> Option<[Span; 3]> {
    span.map_split_range(cx, |scx, range| {
        let s = scx.get_text(range.clone())?;
        let mut trimmed = s;
        ...
        let offset = RelativeBytePos((trimmed.as_ptr().addr() - s.as_ptr().addr()) as u32);
        let pre = range.start..range.start + offset;
        let mid = pre.end..RelativeBytePos(pre.end.0 + trimmed.len() as u32);
        let post = mid.end..range.end;
        Some([pre, mid, post])
    })
}
