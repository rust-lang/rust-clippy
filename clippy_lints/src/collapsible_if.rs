...
use clippy_utils::{
    SpanlessEq, can_use_if_let_chains, is_unit_expr, span_contains_cfg, span_contains_non_whitespace,
    span_extract_comments, sym, tokenize_with_text,
};
    ...
    fn check_collapsible_if_if(
        ...
        if let Some(inner) = expr_block(then)
            ...
            && match (else_, *else_inner) {
                ...
                (Some(a), Some(b)) => {
                    if !SpanlessEq::new(cx).eq_expr(inner.span.ctxt(), a, b) {
                        return;
                    }

                    let comments_a = span_extract_comments(cx, a.span);
                    if !self.lint_commented_code && !comments_a.is_empty() {
                        return;
                    }
                    let comments_b = span_extract_comments(cx, b.span);
