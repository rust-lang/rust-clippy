...
use clippy_utils::{
    expr_path_res, get_item_name, get_parent_expr, higher, in_constant, is_any_item, is_diag_trait_item,
    is_integer_const, iter_input_pats, last_path_segment, paths, unsext, SpanlessEq,
};
    ...
    let (arg_ty, snip) = match expr.kind {
        ...
        ExprKind::Call(path, [arg]) => {
            if expr_path_res(cx, path)
                .opt_def_id()
                .and_then(|id| is_any_item(cx, id, &[&paths::FROM_STR_METHOD[..], &paths::FROM_FROM[..]]))
                .is_some()
            {
