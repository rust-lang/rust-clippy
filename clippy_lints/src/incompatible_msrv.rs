...
declare_clippy_lint! {
    ...
    /// for example if feature in your crate (e.g., `modern_compiler`) should
...
pub struct IncompatibleMsrv {
    msrv: Msrv,
    msrv_for: FxHashMap<DefId, Availability>,
    check_in_tests: bool,
}
...
impl IncompatibleMsrv {
    ...
    fn get_def_id_availability(&mut self, tcx: TyCtxt<'_>, def_id: DefId) -> Availability {
        if let Some(availability) = self.msrv_for.get(&def_id) {
            return *availability;
        }
        ...
        self.msrv_for.insert(def_id, version);
        version
    }
...
fn is_under_cfg_attribute(cx: &LateContext<'_>, hir_id: HirId) -> bool {
    cx.tcx.hir_parent_id_iter(hir_id).any(|id| {
        cx.tcx
            .hir_attrs(id)
            .iter()
            .any(|attr| matches!(attr.name_or_empty(), sym::cfg | sym::cfg_attr))
    })
}
