...
use clippy_utils::{higher, is_integer_literal, path_to_local, peel_blocks_with_stmt, SpanlessEq};
...
fn check_subtraction(
    ...
    if is_integer_literal(if_block.peel_blocks(), 0) {
        // We need to check this case as well to prevent infinite recursion.
        if is_integer_literal(else_block.peel_blocks(), 0) {
    ...
    let if_block = if_block.peel_blocks();
    if is_integer_literal(else_block.peel_blocks(), 0)
            ...
            && let Some(little_var_snippet) = snippet_opt(cx, little_var.span)
        {
            span_lint_and_then(
                ...
                |diag| {
                    diag.span_note(
                        condition_span,
                        format!("you check that `{big_var_snippet}` is bigger than `{little_var_snippet}`"),
                    );
                    diag.span_note(
                        if_block.span,
                        format!("but then you subtract `{big_var_snippet}` from `{little_var_snippet}`"),
                    );
