    ...
    fn visit_expr(&mut self, e: &'tcx Expr<'_>) {
        ...
        match e.kind {
            ExprKind::Binary(op, lhs, rhs) => {
                let lhs_name = snippet_with_context(self.cx, lhs.span, e.span.ctxt(), "..", &mut app).0;
                let rhs_name = snippet_with_context(self.cx, rhs.span, e.span.ctxt(), "..", &mut app).0;
                ...
                match op.node {
                    ...
                    BinOpKind::Ge | BinOpKind::Gt | BinOpKind::Le | BinOpKind::Lt => {
                        let tmpstr = format!("{}!({lhs_name} {} {rhs_name})", self.assert_string, op.node.as_str());
                        self.suggests.push(tmpstr);
                    },
                    _ => {},
                }
