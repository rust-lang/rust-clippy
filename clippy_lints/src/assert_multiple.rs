...
declare_clippy_lint! {
    ...
    pub ASSERT_MULTIPLE,
    nursery,
    "asserting multiple &&-combined predicates to make it clearer which is failing."
}
...
impl<'tcx> Visitor<'tcx> for AssertVisitor<'tcx, '_> {
    fn visit_expr(&mut self, e: &'tcx Expr<'_>) {
        ...
        if let ExprKind::Binary(op, lhs, rhs) = e.kind {
            ...
            if op.node.is_comparison() || matches!(op.node, BinOpKind::And | BinOpKind::Or) {
                // ignore non-boolean results, can't fix
                match op.node {
                    BinOpKind::And => {
