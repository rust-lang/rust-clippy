...
impl<'tcx> Visitor<'tcx> for AssertVisitor<'tcx, '_> {
    fn visit_expr(&mut self, e: &'tcx Expr<'_>) {
        ...
        // let msgtext = match tmsgtext {
        //         None => String::new(),
        //         Some(x) => if x.is_empty() {String::new()} else
        //           { format!(", {}",snippet(self.cx, x, ".." ).to_string())},
        ...
        if let ExprKind::Binary(op, lhs, rhs) = e.kind {
            ...
            match op.node {
                ...
                BinOpKind::Eq => {
                    let tmpstr = format!("{}_eq!({lhs_name}, {rhs_name}{};", self.assert_string, self.msgtext);
                    self.suggests.push(tmpstr);
                },
                BinOpKind::Ne => {
                    let tmpstr = format!("{}_ne!({lhs_name}, {rhs_name}{};", self.assert_string, self.msgtext);
                    self.suggests.push(tmpstr);
                },
                BinOpKind::Or | BinOpKind::Ge | BinOpKind::Gt | BinOpKind::Le | BinOpKind::Lt => {
                    ...
                    let tmpstr = format!("{}!({stripped}{};", self.assert_string, self.msgtext);
                    self.suggests.push(tmpstr);
                },
                _ => {},
            }
            ...
            let snip = snippet_with_context(self.cx, e.span, self.outerctxt, "..", &mut app).0;
            let tmpstr = format!("{}!({snip}{};", self.assert_string, self.msgtext);
            self.suggests.push(tmpstr);
        }
    }
}

// fn convert_msg_args_to_string(pc: &PanicCal) -> Option<Span> {
//         let val = match pc {
//             PanicCall::DefaultMessage => None,
//             PanicCall::Display(e)| PanicCall::Str2015(e)| PanicCall::Format(e) => {
//                 Some(e.span)
//             },
...
impl<'tcx> LateLintPass<'tcx> for AssertMultiple {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, e: &'tcx Expr<'tcx>) {
        ...
        if let Some((condition, _)) = find_assert_args(cx, e, macro_call.expn)
            && matches!(condition.kind, ExprKind::Binary(binop, _lhs, _rhs) if binop.node == BinOpKind::And)
        {
            ...
            let msgspan = macro_call.span.with_lo(condition.span.hi());
            let msgtext: std::borrow::Cow<'_, str> = snippet(cx, msgspan, "..");
            let mut am_visitor = AssertVisitor {
                cx,
                suggests: Vec::new(),
                assert_string,
                outerctxt: condition.span.ctxt(),
                msgtext: msgtext.to_string(),
            };
