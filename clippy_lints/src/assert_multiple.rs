    ...
    fn visit_expr(&mut self, e: &'tcx Expr<'_>) {
        ...
        match e.kind {
            ExprKind::Binary(op, lhs, rhs) => {
                let lhs_name = snippet_with_context(self.cx, lhs.span, self.outerspan.ctxt(), "..", &mut app).0;
                let rhs_name = snippet_with_context(self.cx, rhs.span, self.outerspan.ctxt(), "..", &mut app).0;
                ...
                match op.node {
                    ...
                    BinOpKind::Or | BinOpKind::Ge | BinOpKind::Gt | BinOpKind::Le | BinOpKind::Lt => {
                        let snip = snippet_with_context(self.cx, e.span, self.outerspan.ctxt(), "..", &mut app).0;
            ...
            ExprKind::Unary(UnOp::Not, expr) => {
                // a not clause, just output the expression
                let exptext = snippet_with_context(self.cx, expr.span, self.outerspan.ctxt(), "_", &mut app).0;
