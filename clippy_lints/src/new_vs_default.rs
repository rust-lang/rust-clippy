...
use rustc_hir::{Attribute, Constness, ExprKind, HirIdMap, ImplItemKind, ItemKind, StmtKind};
...
impl_lint_pass!(NewVsDefault => [DEFAULT_MISMATCHES_NEW, NEW_WITHOUT_DEFAULT]);
    ...
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx hir::Item<'_>) {
            ...
            .filter_by_name_unhygienic(sym::new)
        {
            if let AssocKind::Fn { has_self: false, .. } = assoc_item.kind
                ...
                && let ImplItemKind::Fn(ref sig, body_id) = impl_item.kind
                ...
                && let Some(default_trait_id) = cx.tcx.get_diagnostic_item(sym::Default)
            {
                ...
                match default_type {
                    Some(DefaultType::AutoDerived) => {
                        // Don't lint when new() is const - user may need const fn and derived
                        ...
                        if let ExprKind::Block(block, _) = cx.tcx.hir_body(body_id).value.kind
                            && !is_unit_struct(self_ty)
                            && generics.params.is_empty()
                            && let Some(span) = block_span_unless_only_default(cx, block)
                        {
...
/// Returns `Some(span)` if the block contains custom logic (not just a call to `default()`).
/// Returns `None` if the block only contains `Self::default()` or `return Self::default();`.
fn block_span_unless_only_default(cx: &LateContext<'_>, block: &hir::Block<'_>) -> Option<Span> {
    if let Some(expr) = block.expr
        && block.stmts.is_empty()
        && expr_calls_default(cx, expr)
    {
...
fn expr_calls_default(cx: &LateContext<'_>, expr: &hir::Expr<'_>) -> bool {
