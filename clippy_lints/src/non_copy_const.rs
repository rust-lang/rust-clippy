...
use rustc_middle::ty::{
    self, AliasTyKind, EarlyBinder, GenericArgs, GenericArgsRef, Instance, ParamEnv, Ty, TyCtxt, TypeFolder,
    TypeSuperFoldable, TypeckResults,
};
...
impl<'tcx> NonCopyConst<'tcx> {
    ...
    fn is_ty_freeze(&mut self, tcx: TyCtxt<'tcx>, param_env: ParamEnv<'tcx>, ty: Ty<'tcx>) -> IsFreeze {
                ...
                let is_freeze = match *ty.kind() {
                    ...
                    ty::Adt(adt, args) if adt.is_enum() => IsFreeze::from_variants(adt.variants().iter().map(|v| {
                        IsFreeze::from_fields(
                            v.fields
                                .iter()
                                .map(|f| self.is_ty_freeze(tcx, param_env, f.ty(tcx, args))),
                        )
                    })),
                    ...
                    ty::Adt(adt, args)
                        if adt.is_union()
                            && adt.non_enum_variant().fields.iter().any(|f| {
                                tcx.layout_of(param_env.and(f.ty(tcx, args)))
                    ...
                    ty::Array(ty, _) | ty::Pat(ty, _) => self.is_ty_freeze(tcx, param_env, ty),
                    ty::Tuple(tys) => IsFreeze::from_fields(tys.iter().map(|ty| self.is_ty_freeze(tcx, param_env, ty))),
    ...
    fn is_value_freeze(
        ...
        let ty = tcx.try_normalize_erasing_regions(param_env, ty).unwrap_or(ty);
        match self.is_ty_freeze(tcx, param_env, ty) {
            ...
            IsFreeze::Maybe if matches!(ty.kind(), ty::Adt(..) | ty::Array(..) | ty::Tuple(..)) => {
                for &(val, ty) in tcx
                    ...
                    .fields
                {
                    if !self.is_value_freeze(tcx, param_env, ty, val)? {
                        return Ok(false);
                    }
                }
                Ok(true)
    ...
    ///
    /// `param_env` and `gen_args` are from the constant's use site.
            ...
            IsFreeze::Maybe => match e.kind {
                ExprKind::Block(b, _)
                    if !b.targeted_by_break
                        ...
                        && let Some(e) = b.expr =>
                {
                    self.is_init_expr_freeze(tcx, param_env, typeck, gen_args, e)
                },
                ExprKind::Path(ref p) => {
                    ...
                    match res {
                        Res::Def(DefKind::Const | DefKind::AssocConst, did)
                            if let Ok(val) =
                                tcx.const_eval_resolve(param_env, UnevaluatedConst::new(did, gen_args), DUMMY_SP)
                                && let Ok(is_freeze) = self.is_value_freeze(tcx, param_env, ty, val) =>
                        {
                            is_freeze
                        },
                        Res::Def(DefKind::Const | DefKind::AssocConst, did)
                            if let Some((typeck, init)) = get_const_hir_value(tcx, param_env, did, gen_args) =>
                        {
                            self.is_init_expr_freeze(tcx, param_env, typeck, gen_args, init)
                        },
                ...
                {
                    args.iter()
                        .all(|e| self.is_init_expr_freeze(tcx, param_env, typeck, gen_args, e))
                },
                ExprKind::Struct(_, fields, None) => fields
                    .iter()
                    .all(|f| self.is_init_expr_freeze(tcx, param_env, typeck, gen_args, f.expr)),
                ExprKind::Tup(exprs) | ExprKind::Array(exprs) => exprs
                    .iter()
                    .all(|e| self.is_init_expr_freeze(tcx, param_env, typeck, gen_args, e)),
                ExprKind::Repeat(e, _) => self.is_init_expr_freeze(tcx, param_env, typeck, gen_args, e),
                _ => true,
            },
        }
    }
    ...
    fn is_non_freeze_expr_borrowed(
        ...
        let mut parents = tcx.hir().parent_iter(src_expr.hir_id);
        loop {
            let ty = typeck.expr_ty(src_expr);
            // Normalized as we need to check if this is an array later.
            let ty = tcx.try_normalize_erasing_regions(param_env, ty).unwrap_or(ty);
            let is_freeze = self.is_ty_freeze(tcx, param_env, ty);
            if is_freeze.is_freeze() {
                return None;
            }
        ...
        loop {
            ...
            if let [adjust, ..] = typeck.expr_adjustments(src_expr) {
                let res = if let Some(cause) = does_adjust_borrow(adjust)
                    && !self.is_value_freeze(tcx, param_env, ty, val)?
                {
            ...
            if self.is_ty_freeze(tcx, param_env, ty).is_freeze() {
                return Ok(None);
            }
            let Some((_, Node::Expr(use_expr))) = parents.next() else {
                return Ok(None);
            ...
            let next_val = match use_expr.kind {
                ...
                ExprKind::Index(_, idx, _) if ty.is_array() => {
                    ...
                    if let Some(Constant::Int(idx)) = ConstEvalCtxt::with_env(tcx, param_env, typeck).eval(idx) {
                        val.fields.get(idx as usize)
                    } else {
                        // It's some value in the array so check all of them.
                        for &(val, _) in val.fields {
                            if let Some(src) = self.is_non_freeze_val_borrowed(tcx, param_env, typeck, use_expr, val)? {
                                return Ok(Some(src));
                            }
                        }
                        return Ok(None);
                ...
                ExprKind::AddrOf(..) if !self.is_value_freeze(tcx, param_env, ty, val)? => {
    ...
    ///
    /// `param_env` and `init_args` are from the constant's use site.
    ...
    fn is_non_freeze_init_borrowed(
        ...
        loop {
            ...
            if let [adjust, ..] = typeck.expr_adjustments(src_expr) {
                return does_adjust_borrow(adjust)
                    .filter(|_| !self.is_init_expr_freeze(tcx, param_env, init_typeck, init_args, init_expr))
                    .map(|cause| BorrowSource::new(tcx, src_expr, cause));
            }
                        ...
                        match init_typeck.qpath_res(init_path, init_expr.hir_id) {
                            ...
                            Res::Def(DefKind::Const | DefKind::AssocConst, did)
                                if let Ok(val) = tcx.const_eval_resolve(
                                    ...
                                    self.is_non_freeze_val_borrowed(tcx, param_env, init_typeck, src_expr, val) =>
                            {
                                return res;
                            },
                            Res::Def(DefKind::Const | DefKind::AssocConst, did)
                                if let Some((next_typeck, value)) =
                                    get_const_hir_value(tcx, param_env, did, next_init_args) =>
                            {
                            ...
                            // type based check.
                            _ => {
                                return self.is_non_freeze_expr_borrowed(tcx, param_env, typeck, src_expr);
                            },
                        }
                    },
                    _ => break,
            ...
            init_expr = match &use_expr.kind {
                ExprKind::Field(_, name) => match init_expr.kind {
                    ...
                    _ => return self.is_non_freeze_expr_borrowed(tcx, param_env, typeck, use_expr),
                },
                ...
                ExprKind::AddrOf(..)
                    if !self.is_init_expr_freeze(tcx, param_env, init_typeck, init_args, init_expr) =>
                {
                    return Some(BorrowSource::new(tcx, use_expr, BorrowCause::Borrow));
                },
                // All other expressions use the value.
...
impl<'tcx> LateLintPass<'tcx> for NonCopyConst<'tcx> {
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx Item<'_>) {
        if let ItemKind::Const(.., body_id) = item.kind
            && !item.ident.is_special()
            && let ty = cx.tcx.type_of(item.owner_id).instantiate_identity()
            && match self.is_ty_freeze(cx.tcx, cx.param_env, ty) {
                IsFreeze::No => true,
                IsFreeze::Yes => false,
                IsFreeze::Maybe => match cx.tcx.const_eval_poly(item.owner_id.to_def_id()) {
                    Ok(val) if let Ok(is_freeeze) = self.is_value_freeze(cx.tcx, cx.param_env, ty, val) => !is_freeeze,
                    _ => !self.is_init_expr_freeze(
                        cx.tcx,
                        cx.param_env,
                        cx.tcx.typeck(item.owner_id),
                        GenericArgs::identity_for_item(cx.tcx, item.owner_id),
                        cx.tcx.hir().body(body_id).value,
                    ),
                },
            }
            ...
            && match self.is_ty_freeze(cx.tcx, cx.param_env, ty) {
                ...
                IsFreeze::Maybe if let Some(body_id) = body_id_opt => {
                    match cx.tcx.const_eval_poly(item.owner_id.to_def_id()) {
                        Ok(val) if let Ok(is_freeeze) = self.is_value_freeze(cx.tcx, cx.param_env, ty, val) => {
                            !is_freeeze
                        },
                        _ => !self.is_init_expr_freeze(
                            cx.tcx,
            ...
            && match self.is_ty_freeze(cx.tcx, cx.param_env, ty) {
                ...
                IsFreeze::No => {
                        ...
                        && let Some(trait_id) = of_trait.trait_def_id()
                    {
                        ...
                        !self.is_ty_freeze(cx.tcx, cx.param_env, ty).is_not_freeze()
                    } else {
                        true
                    }
                },
    ...
    fn check_expr(&mut self, cx: &LateContext<'tcx>, e: &'tcx Expr<'_>) {
        if let ExprKind::Path(qpath) = &e.kind
            ...
            && !self.is_ty_freeze(cx.tcx, cx.param_env, typeck.expr_ty(e)).is_freeze()
            && let Some(borrow_src) = {
                ...
                if let Ok(val) = cx.tcx.const_eval_resolve(
                    ...
                    DUMMY_SP,
                ) && let Ok(src) = self.is_non_freeze_val_borrowed(cx.tcx, cx.param_env, typeck, e, val)
                {
                    src
                } else if let init_args = typeck.node_args(e.hir_id)
                    && let Some((init_typeck, init)) = get_const_hir_value(cx.tcx, cx.param_env, did, init_args)
                {
                    self.is_non_freeze_init_borrowed(cx.tcx, cx.param_env, typeck, e, init_typeck, init_args, init)
                } else {
                    self.is_non_freeze_expr_borrowed(cx.tcx, cx.param_env, typeck, e)
                }
            }
...
fn get_const_hir_value<'tcx>(
    ...
    let (did, body_id) = match tcx.hir_node(tcx.local_def_id_to_hir_id(did)) {
        ...
        Node::TraitItem(_)
            if let Ok(Some(inst)) = Instance::try_resolve(tcx, param_env, did.into(), args)
    ...
    Some((tcx.typeck(did), tcx.hir().body(body_id).value))
}
