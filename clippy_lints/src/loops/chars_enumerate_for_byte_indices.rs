...
pub(super) fn check<'tcx>(cx: &LateContext<'tcx>, pat: &Pat<'_>, iterable: &Expr<'_>, body: &'tcx Expr<'tcx>) {
    if let ExprKind::MethodCall(_, recv, _, enumerate_span) = iterable.kind
        ...
        && let ExprKind::MethodCall(chars_segment, recv, _, chars_span) = recv.kind
            ...
            && let PatKind::Binding(_, binding_id, ..) = pat.kind
        {
            ...
            for_each_expr(cx, body, |expr| {
                if path_to_local_id(expr, binding_id) {
                    check_index_usage(cx, expr, pat, enumerate_span, chars_span, recv);
                }
                CONTINUE
            });
