...
impl ItemNameRepetitions {
    ...
    pub fn new(
        ...
        Self {
            ...
            allowed_prepositions: allowed_prepositions.iter().map(|s| to_camel_case(s)).collect(),
        }
    }

    fn is_allowed_preposition(&self, prefix: &str) -> bool {
        self.allowed_prepositions.contains(prefix)
    }
}
                ...
                if !mod_camel.is_empty() {
                    ...
                    if cx.tcx.visibility(item.owner_id).is_public() && item_camel.len() > mod_camel.len() {
                        ...
                        if rmatching.char_count == nchars
                            && !self.is_allowed_preposition(&item_camel[..item_camel.len() - rmatching.byte_count])
                        {
                            span_lint(
                                cx,
                                MODULE_NAME_REPETITIONS,
