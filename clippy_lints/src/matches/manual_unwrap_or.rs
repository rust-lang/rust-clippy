...
fn handle(
    ...
    let (body_some, body_none) = (peel_blocks(body_some), peel_blocks(body_none));

    let expr_type = cx.typeck_results().expr_ty(expr);
    // We check that the `Some(x) => x` doesn't do anything apart "returning" the value in `Some`.
    if let ExprKind::Path(QPath::Resolved(_, path)) = body_some.kind
        ...
        && local_id == binding_id
    {
        ...
        let receiver = Sugg::hir_with_applicability(cx, condition, "_", &mut applicability).maybe_par();
            ...
            && is_default_equivalent(cx, body_none)
        {
            ...
            && let Some(or_body_snippet) = body_none.span.get_source_text(cx)
            ...
            && ConstEvalCtxt::new(cx).eval_simple(body_none).is_some()
        {
            ...
            let suggestion = Sugg::hir_with_context(cx, condition, expr.span.ctxt(), "..", &mut app).maybe_par();
            span_lint_and_sugg(
                cx,
                MANUAL_UNWRAP_OR,
                expr.span,
...
pub fn check_if_let<'tcx>(
    ...
    if let Some(binding_id) = get_some(cx, pat) {
        handle(cx, expr, "if let", scrutinee, then_expr, else_expr, binding_id);
    }
}
