...
declare_clippy_lint! {
    ...
    /// #[derive(PartialEq)]
    /// struct Number(f32);
    /// // Incorrect because `PartialEq` derives `==` equality.
    /// // Because of `f32::NaN != f32::NaN`, so `a != a` reflexivity of `Number` is not possible.
    /// // Therefore, it should not implement `Eq`.
