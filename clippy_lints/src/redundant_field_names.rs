            ...
            && self.msrv.meets(msrvs::FIELD_INIT_SHORTHAND)
        {
            for field in &se.fields {
                if !field.is_shorthand
                    ...
                    // The whole expression may be generated by an external macro
                    // (e.g. a `#[derive]`-generated `From` impl), while the field
                    // name spans still point back at the user's struct
                    // definition, so the field-level check alone is not enough (#17525).
