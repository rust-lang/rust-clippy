...
use rustc_hir::{Expr, ExprKind, HirId, PathSegment};
...
impl<'tcx> LateLintPass<'tcx> for ClonedRefToSliceRefs<'_> {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &Expr<'tcx>) {
        if self.msrv.meets(cx, {
            ...
            && let Some(adjustment) = is_needless_clone_or_equivalent(cx, recv, path, item.hir_id)
...
/// let b = &[w.to_path_buf()];
/// ```
/// To use the `w` directly without cloning, the above code needs to be adjusted to`&*w`.
fn is_needless_clone_or_equivalent<'tcx>(
    ...
    if !method_def.is_diag_item(cx, sym::Clone) && !is_clone_like(cx, method_path.ident.name, method_def) {
        return None;
    }
    ...
    let adjust_target_ty = if method_ret_ty == *method_recv_ty_inner {
        ...
        && matches!(
            (after_special_case_ty_name, before_special_case_ty.opt_diag_name(cx)?),
            (sym::OsStr, sym::OsString) | (sym::Path, sym::PathBuf))
    {
