...
declare_clippy_lint! {
    /// ### What it does
    /// Checks for explicit `deref()` or `deref_mut()` method calls that occur outside the implementation of the `Deref` or `DerefMut` traits.
    ///
    /// ### Why is this bad?
...
pub struct Dereferencing<'tcx> {
    ...
    /// Tracks the nesting depth of `Deref` or `DerefMut` trait implementations.
    /// This counter is incremented when entering such an impl block and decremented when exiting.
    /// A non-zero value indicates we are currently inside one or more nested impl blocks.
    deref_impl_nesting_depth: u32,
}
...
impl<'tcx> LateLintPass<'tcx> for Dereferencing<'tcx> {
    ...
    fn check_item(&mut self, cx: &LateContext<'_>, item: &Item<'_>) {
        if is_deref_or_deref_mut_impl(cx, item) {
            self.deref_impl_nesting_depth += 1;
        }
    }
    ...
    fn report(
        ...
        match state {
            State::DerefMethod {
                ...
                if self.deref_impl_nesting_depth > 0 {
                    return;
                }
