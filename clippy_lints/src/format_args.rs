...
/// For a `#[clippy::format_args]` macro that wraps its argument before forwarding to
/// `format_args!` (e.g. `Adapter(&($arg))`), find the single call-site simple path that `$arg`
/// was substituted with.
///
/// Returns `(name, span)` only when exactly one call-site simple path is found within `expr`
/// (a span that is NOT from a macro expansion, meaning it came directly from the call site).
fn find_callsite_simple_path(expr: &rustc_ast::Expr) -> Option<(Symbol, Span)> {
    match &expr.kind {
        // Target: a simple path directly from the call site (not generated by the macro body).
        rustc_ast::ExprKind::Path(None, path)
            if !expr.span.from_expansion()
                && let [seg] = path.segments.as_slice()
                && seg.args.is_none() =>
        {
            Some((seg.ident.name, expr.span))
        },
        // `Paren` is always transparent: `($arg)` in a rule body produces a `Paren` whose
        // span may or may not be from expansion depending on token provenance, so always look
        // inside without checking `from_expansion`.
        rustc_ast::ExprKind::Paren(inner) => find_callsite_simple_path(inner),
        // Macro-body wrappers: only recurse when the expression itself was synthesized by the
        // macro (i.e. the adapter pattern like `Adapter(&($arg))`).
        rustc_ast::ExprKind::AddrOf(_, _, inner) if expr.span.from_expansion() => find_callsite_simple_path(inner),
        rustc_ast::ExprKind::Unary(_, inner) if expr.span.from_expansion() => find_callsite_simple_path(inner),
        rustc_ast::ExprKind::Cast(inner, _) if expr.span.from_expansion() => find_callsite_simple_path(inner),
        rustc_ast::ExprKind::Call(_, args) if expr.span.from_expansion() => {
            let mut found = None;
            for arg in args {
                if let Some(result) = find_callsite_simple_path(arg) {
                    if found.is_some() {
                        return None; // multiple call-site paths — ambiguous
                    }
                    found = Some(result);
                }
            }
        ...
        // Call-site non-path expression (e.g. `*ptr`, `foo.bar`, `x + 1`) — can't inline.
        _ => None,
    }
}
...
impl<'tcx> FormatArgsExpr<'_, 'tcx> {
    ...
    fn check_one_arg(&self, pos: &FormatArgPosition, usage: FormatParamUsage, fixes: &mut Vec<(Span, String)>) -> bool {
            ...
            && let Some(pos_span) = pos.span
        {
            ...
            && let Some((var_name, var_span)) = find_callsite_simple_path(&arg.expr)
            && let Some(pos_span) = pos.span
        {
            // The arg expression is from a `#[clippy::format_args]` macro that adapts its
            // arguments before forwarding to `format_args!` (e.g. wraps `x` in `Adapter(&x)`).
            // `var_span` is the call-site span of the original simple variable.
            let prev_hi = if index == 0 {
                self.format_args.span.hi()
            } else {
                let Some(prev_arg) = self.format_args.arguments.by_index(index - 1) else {
                    return true;
                };
                // Get the call-site hi of the previous arg, which was also inlineable.
                if let Some((_, prev_span)) = find_callsite_simple_path(&prev_arg.expr) {
                    prev_span.hi()
                } else {
                    return true;
                }
