...
pub(crate) fn check<'tcx>(cx: &LateContext<'tcx>, expr: &Expr<'_>, recv: &'tcx Expr<'tcx>, iter_method: &str) {
    ...
    let caller_type = if derefs_to_slice(cx, recv, ty).is_some() {
        ...
        "Vec"
    } else if is_item(cx, ty, sym::vecdeque_type) {
        "VecDeque"
    } else if is_item(cx, ty, sym::hashset_type) {
        "HashSet"
    } else if is_item(cx, ty, sym::hashmap_type) {
        "HashMap"
    } else if is_item(cx, ty, sym::BTreeMap) {
        "BTreeMap"
    } else if is_item(cx, ty, sym::BTreeSet) {
        "BTreeSet"
    } else if is_item(cx, ty, sym::LinkedList) {
        "LinkedList"
    } else if is_item(cx, ty, sym::BinaryHeap) {
        "BinaryHeap"
    } else {
        return;
    };
