...
use clippy_utils::ty::{implements_trait, peel_middle_ty_refs, should_call_clone_as_function};
...
pub(super) fn check(cx: &LateContext<'_>, expr: &hir::Expr<'_>, call_name: Symbol, recvr: &hir::Expr<'_>) {
    ...
    if is_diag_trait_item(cx, def_id, sym::AsRef) || is_diag_trait_item(cx, def_id, sym::AsMut) {
        ...
        let (base_res_ty, res_depth) = peel_middle_ty_refs(res_ty);
        let (base_rcv_ty, rcv_depth) = peel_middle_ty_refs(rcv_ty);
