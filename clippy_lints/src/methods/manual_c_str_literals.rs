...
pub(super) fn check_as_ptr<'tcx>(
    ...
    {
        let is_already_casted = casts_removed.hir_id != expr.hir_id;
        ...
        span_lint_and_then(
            ...
            |diag| {
                ...
                if !is_already_casted {
                    diag.note(
                        "an additional cast may be needed, since the type of `CStr::as_ptr` \
                        and `\"\".as_ptr()` can differ and is platform dependent",
                    );
                }
            },
        );
