...
pub(crate) fn check(cx: &LateContext<'_>, expr: &Expr<'_>, arg: &Expr<'_>, name: &str, name_span: Span, msrv: Msrv) {
        ...
        && typeck.expr_adjustments(final_expr).is_empty()
    {
        ...
        for x in delayed {
            match x {
                UseKind::Return(s) => {
                    if let Some(s) = s.map_span(cx, SourceFileRange::add_leading_whitespace) {
        ...
        if (!requires_copy || cx.type_is_copy_modulo_regions(arg_ty))
            ...
            && let Some(final_expr_span) = final_expr.span.map_span(cx, SourceFileRange::add_leading_whitespace)
        {
            if requires_deref {
