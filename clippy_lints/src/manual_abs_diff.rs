...
impl<'tcx> LateLintPass<'tcx> for ManualAbsDiff {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        if !expr.span.from_expansion()
            ...
            && Self::is_sub_expr(cx, if_expr.then, a, b, ty)
            && Self::is_sub_expr(cx, r#else, b, a, ty)
        {
            span_lint_and_then(
                cx,
                MANUAL_ABS_DIFF,
                ...
                |diag| {
                    let (has_certain_ty, a, b) = match (expr_type_is_certain(cx, a), expr_type_is_certain(cx, b)) {
                        (false, false) => (false, a, b),
                        (false, true) => (true, b, a),
                        (true, _) => (true, a, b),
                    };
                    let applicability = if has_certain_ty {
...
impl ManualAbsDiff {
    ...
    fn is_sub_expr(
        ...
        if let ty::Int(ty) = expected_ty.kind() {
            ...
            {
                Self::is_sub_expr(cx, expr, expected_a, expected_b, unsigned)
            } else {
                false
            };
        }
