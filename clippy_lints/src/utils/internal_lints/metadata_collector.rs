...
{}"#,
            self.config
                .iter()
                .filter(|x| x.renamed_to.is_none())
                .format_with("\n", |x, f| f(&x.display_markdown_paragraph()))
        )
    }
}
...
{}",
        ...
        writeln!(
            ...
            &changelog[..position],
            self.config.iter().filter(|x| x.renamed_to.is_none())
            .format_with("\n", |x, f| f(&x.display_markdown_link()))
        )
        .unwrap();
    }
}
