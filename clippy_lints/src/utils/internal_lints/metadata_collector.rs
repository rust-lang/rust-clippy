...
"#
    ...
    fn write_unsafe_lints(&self) -> Result<(), Box<dyn Error>> {
        let f = std::fs::File::create("unsafe-lints")?;
        let mut buf = std::io::BufWriter::new(f);
        writeln!(
            buf,
            "<< Generated by cargo collect-metadata. DO NOT EDIT MANUALLY NOR DELETE>>"
        )?;
        self.lints.iter().for_each(|lint| {
            if lint.is_unsafe() && lint.group != "internal" {
                writeln!(buf, "{}", lint.id).unwrap();
            }
        });
        Ok(())
    }
...
impl Drop for MetadataCollector {
    ...
    fn drop(&mut self) {
        ...
        self.write_unsafe_lints().unwrap();
...
impl LintMetadata {
    ...
    /// Returns `true` if there's an "Known problems" section, meaning that there may be problems
    /// when applying it using `--fix`
    fn is_unsafe(&self) -> bool {
        // dbg!(self.docs);
        self.docs.to_lowercase().contains("# known problem")
        // A single "#" because known problems may be ## and others ###. + "problem" in singular for
        // more compatibility
    }
}
