...
use clippy_utils::{msrvs, peel_hir_expr_refs};
...
impl LateLintPass<'_> for ManualOptionAsSlice {
    ...
    fn check_expr(&mut self, cx: &LateContext<'_>, expr: &Expr<'_>) {
        if !self.msrv.meets(msrvs::OPTION_AS_SLICE) {
            return;
        }
        let span = expr.span;
        match expr.kind {
            ExprKind::Match(scrutinee, [arm1, arm2], _) => {
                if is_none_arm(cx, arm1) {
                    if check_arms(cx, arm1, arm2) {
                        check_as_ref(cx, scrutinee, span);
                    }
                } else if is_none_arm(cx, arm2) && check_arms(cx, arm2, arm1) {
                    check_as_ref(cx, scrutinee, span);
                }
            },
            ExprKind::If(cond, then, Some(other)) => {
                if let ExprKind::Let(let_expr) = cond.kind
                    && let Some(binding) = check_some_pat(cx, let_expr.pat)
...
fn is_none_arm(cx: &LateContext<'_>, arm: &Arm<'_>) -> bool {
    if let PatKind::Path(QPath::Resolved(None, path)) = &arm.pat.kind
        && let Res::Def(DefKind::Ctor(_, _), def_id) = path.res
        && clippy_utils::is_lang_item_or_ctor(cx, def_id, LangItem::OptionNone)
    {
        true
    } else {
        false
...
fn check_some_pat(cx: &LateContext<'_>, pat: &Pat<'_>) -> Option<Symbol> {
...
fn check_arms(cx: &LateContext<'_>, none_arm: &Arm<'_>, some_arm: &Arm<'_>) -> bool {
    if none_arm.guard.is_none()
        ...
        && let Some(name) = check_some_pat(cx, some_arm.pat)
    {
...
fn returns_empty_slice(cx: &LateContext<'_>, expr: &Expr<'_>) -> bool {
    match expr.kind {
        ...
        ExprKind::Closure(cl) => {
            let body = cx.tcx.hir().body(cl.body);
            is_empty_slice(cx, body.value)
        }, //TODO
        _ => false,
    }
}
