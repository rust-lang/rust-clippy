...
use rustc_hir::{Expr, ExprKind, Item, ItemKind, QPath, Variant, VariantData};
...
impl EmptyWithBrackets {
    ...
    fn check_def(
        ...
        let (fields, has_semi, start_char, end_char) = match *data {
            VariantData::Struct { fields, .. } => (fields, false, '{', '}'),
            VariantData::Tuple(fields, _, _) => (fields, needs_semi, '(', ')'),
        ...
        if fields.is_empty()
            ...
            && let Some(lint_range) = (name_hi..item_sp.hi()).clone().map_range(cx, |_, src, range| {
                ...
                // Skip leading whitespace for the lint span.
                let trimmed = src.trim_start();
                let start = range.start + (src.len() - trimmed.len());
                // Make sure no unexpected text occurs.
                trimmed
                    .strip_prefix(start_char)?
                    .trim_start()
        ...
        {
            let edit_sp = (name_hi..lint_range.end).into_span();
            let lint_sp = lint_range.into_span();
            let needs_semi = needs_semi && !has_semi;
            if start_char == '{' {
