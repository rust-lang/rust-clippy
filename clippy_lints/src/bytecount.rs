...
impl<'tcx> LateLintPass<'tcx> for ByteCount {
    fn check_expr(&mut self, cx: &LateContext<'_>, expr: &Expr<'_>) {
        if let ExprKind::MethodCall(count, [count_recv], _) = expr.kind
            && count.ident.name == sym::count
            && let ExprKind::MethodCall(filter, [filter_recv, filter_arg], _) = count_recv.kind
            && filter.ident.name == sym!(filter)
            && let ExprKind::Closure(_, _, body_id, _, _) = filter_arg.kind
        ...
        {
            let haystack = if let ExprKind::MethodCall(path, args, _) = filter_recv.kind {
