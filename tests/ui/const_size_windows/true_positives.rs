...
fn windows_const_mod_path_size(slice: &[u8]) {
    // expected: `slice.array_windows::<{ some_module::WINDOW_SIZE }>()`
    for pair in slice.windows(some_module::WINDOW_SIZE) {
        //~^ const_size_windows
        println!("{} {}", pair[0], pair[1]);
    }
}

fn windows_const_computed_size(slice: &[u8]) {
    // expected: `slice.array_windows::<{ 2 + 2 }>()`
    for pair in slice.windows(2 + 2) {
        //~^ const_size_windows
        println!("{} {}", pair[0], pair[1]);
    }
}

fn collect_after_windows(slice: &[u8]) {
    // expected: `slice.array_windows::<2>().collect::<Vec<_>>()`
    let windows = slice.windows(2).collect::<Vec<_>>();
    //~^ const_size_windows
