...
fn main() {
    ...
    let _ = str.splitn(2, '=').next(); //~ needless_splitn
    let _ = str.splitn(2, '=').nth(0); //~ needless_splitn
    ...
    let _ = str.rsplitn(2, '=').next(); //~ needless_splitn
    let _ = str.rsplitn(2, '=').nth(0); //~ needless_splitn
    let _ = str.rsplitn(2, '=').nth(1);
    let (_, _) = str.rsplitn(2, '=').next_tuple().unwrap();
    let (_, _) = str.rsplitn(3, '=').next_tuple().unwrap();
    //~^ needless_splitn

    let _ = str.splitn(5, '=').next(); //~ needless_splitn
    let _ = str.splitn(5, '=').nth(3); //~ needless_splitn
    let _ = str.splitn(5, '=').nth(4);
    let _ = str.splitn(5, '=').nth(5);
}

fn _question_mark(s: &str) -> Option<()> {
    let _ = s.splitn(2, '=').next()?; //~ needless_splitn
    let _ = s.splitn(2, '=').nth(0)?; //~ needless_splitn
    let _ = s.rsplitn(2, '=').next()?; //~ needless_splitn
    let _ = s.rsplitn(2, '=').nth(0)?; //~ needless_splitn
