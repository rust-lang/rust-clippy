...
macro_rules! simple {
    ($v:expr) => {
        unsafe {
            //~^ ERROR: this unsafe block in a macro expands `expr` metavariables
            dbg!($v);
        }
    };
}
...
// don't lint exported macros that are doc(hidden) because they aren't part of the public API
#[macro_export]
#[doc(hidden)]
...
// unsafe {} is still needed at callsite so not unsound
#[macro_export]
