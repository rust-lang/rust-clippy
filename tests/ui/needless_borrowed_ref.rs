...
fn should_lint(
    ...
    let var = 3;
    let thingy = Some(&var);
    if let Some(&ref v) = thingy {} //~ needless_borrowed_reference

    if let &[&ref a, ref b] = slice_of_refs {}
    //~^ needless_borrowed_reference

    let &[ref a, ..] = &array; //~ needless_borrowed_reference
    ...
    if let &[ref a, _] = slice {} //~ needless_borrowed_reference

    if let &(ref a, ref b, ref c) = &tuple {}
    //~^ needless_borrowed_reference
    if let &(ref a, _, ref c) = &tuple {}
    //~^ needless_borrowed_reference
    if let &(ref a, ..) = &tuple {} //~ needless_borrowed_reference

    if let &TupleStruct(ref a, ..) = &tuple_struct {}
    //~^ needless_borrowed_reference

    //~v needless_borrowed_reference
    if let &Struct {
        ref a,
        b: ref b,
        c: ref renamed,
