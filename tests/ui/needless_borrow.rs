...
fn main() {
    ...
    let garbl = match 42 {
        ...
        46 => &&a, //~ needless_borrow
        47 => {
            println!("foo");
            loop {
                println!("{}", a);
                if a == 25 {
                    break &ref_a; //~ needless_borrow
                }
            }
        },
        _ => panic!(),
    ...
    let _ = x(&&&a); //~ needless_borrow
    let _ = x(&mut &&a); //~ needless_borrow
    let _ = x(&&&mut b); //~ needless_borrow
    let _ = x(&&ref_a); //~ needless_borrow
    {
        let b = &mut b;
        x(&b); //~ needless_borrow
    }
    ...
    mut_ref(&mut x); //~ needless_borrow
    mut_ref(&mut &mut x); //~ needless_borrow
    let y: &mut i32 = &mut x; //~ needless_borrow
    let y: &mut i32 = &mut &mut x; //~ needless_borrow
    ...
    *x = 5;
    ...
    let x = (1, 2);
    let _ = (&x).0; //~ needless_borrow
    let x = &x as *const (i32, i32);
    let _ = unsafe { (&*x).0 }; //~ needless_borrow
...
fn issue9383() {
    ...
    unsafe {
        ...
        let _ = &mut (&mut x.u).x;
        let _ = &mut (&mut { x.u }).x; //~ needless_borrow
        let _ = &mut ({ &mut x.u }).x;
