...
fn main() {
    {
        let _ = [Ok(1), Err(2), Ok(3)].into_iter().filter(Result::is_ok);
        //~^ iter_filter_is_ok

        let _ = [Ok(1), Err(2), Ok(3)].into_iter().filter(|a| a.is_ok());
        //~^ iter_filter_is_ok

        #[rustfmt::skip]
        let _ = [Ok(1), Err(2)].into_iter().filter(|o| { o.is_ok() });
        //~^ iter_filter_is_ok
    }
    ...
    {
        let _ = [Ok(1), Err(2), Ok(3)].into_iter().filter(std::result::Result::is_ok);
        //~^ iter_filter_is_ok

        let _ = [Ok(1), Err(2), Ok(3)]
            .into_iter()
            .filter(|a| std::result::Result::is_ok(a));
        //~^ iter_filter_is_ok

        #[rustfmt::skip]
        let _ = [Ok(1), Err(2), Ok(3)].into_iter().filter(|a| { std::result::Result::is_ok(a) });
        //~^ iter_filter_is_ok
    }

    {
        let _ = [Ok(1), Err(2), Ok(3)].into_iter().filter(|ref a| a.is_ok());
        //~^ iter_filter_is_ok
        ...
        #[rustfmt::skip]
        let _ = [Ok(1), Err(2)].into_iter().filter(|ref o| { o.is_ok() });
        //~^ iter_filter_is_ok
    }
}
...
fn avoid_linting_when_filter_has_side_effects() {
    // Don't lint below
    let mut counter = 0;
    let _ = [Ok(1), Err(2)].into_iter().filter(|o| {
        counter += 1;
        o.is_ok()
    });
}
...
fn ice_12058() {
    ...
    Some(1).or_else(|| {
        [Ok(1), Err(())].into_iter().filter(|z| *z != Ok(2));
        None
    });
}
...
fn avoid_linting_map() {
    // should not lint
    let _ = [Ok(1), Err(())].into_iter().filter(|o| o.is_ok()).map(|o| o.unwrap());

    // should not lint
    let _ = [Ok(1), Err(())].into_iter().filter(|o| o.is_ok()).map(|o| o);
}
...
fn avoid_false_positive_due_to_method_name() {
    ...
    [Ok(1), Err(2), Ok(3)].into_iter().filter(is_ok);
    // should not lint
}
...
fn avoid_fp_due_to_trait_type() {
    ...
    [Ok(1), Err(2), Ok(3)].into_iter().filter(Foo::is_ok);
    // should not lint
}
