...
// Should not warn: the `?` targets the `try` block and would no longer compile
// inside the closure of a `bool::then` call.
fn issue17596(x: u32) -> Option<Option<u32>> {
    try { if x > 0 { Some(x.checked_sub(2)?) } else { None } }
}
