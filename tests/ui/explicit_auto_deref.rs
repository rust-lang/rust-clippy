...
fn main() {
    ...
    struct S1<'a>(&'a str);
    let _ = S1(&*s); //~ explicit_auto_deref

    struct S2<'a> {
        s: &'a str,
    }
    let _ = S2 { s: &*s }; //~ explicit_auto_deref
    ...
    impl<'a> E1<'a> {
        fn m1(s: &'a String) {
            let _ = Self::S1(&**s); //~ explicit_auto_deref
            let _ = Self::S2 { s: &**s }; //~ explicit_auto_deref
        }
    }
    let _ = E1::S1(&*s); //~ explicit_auto_deref
    let _ = E1::S2 { s: &*s }; //~ explicit_auto_deref
    ...
    let _ = b.foo;
    let _ = (*b).foo; //~ explicit_auto_deref
    let _ = (**b).foo; //~ explicit_auto_deref
    ...
    let ref_str = &"foo";
    let _ = f_str(*ref_str); //~ explicit_auto_deref
    let ref_ref_str = &ref_str;
    let _ = f_str(**ref_ref_str); //~ explicit_auto_deref
    ...
    let c1 = |_: &Vec<&u32>| {};
    let x = &&vec![&1u32];
    c1(*x); //~ explicit_auto_deref
    let _ = for<'a, 'b> |x: &'a &'a Vec<&'b u32>, b: bool| -> &'a Vec<&'b u32> {
        if b {
            return *x; //~ explicit_auto_deref
        }
        *x //~ explicit_auto_deref
    };
    ...
    let _: &mut u32 = match &mut Some(&mut 0u32) {
        Some(x) => &mut *x, //~ explicit_auto_deref
        None => panic!(),
    };
