...
fn main() {
    ...
    let _ = s.splitn(2, '=').nth(1).unwrap(); //~ manual_split_once
    ...
    fn _f(s: &str) -> Option<&str> {
        let _ = s.splitn(2, '=').nth(1)?; //~ manual_split_once
        let _ = s.splitn(2, '=').skip(1).next()?;
        //~^ manual_split_once
        let _ = s.rsplitn(2, '=').nth(1)?; //~ manual_split_once
...
fn indirect() -> Option<()> {
    let mut iter = "a.b.c".splitn(2, '.'); //~ manual_split_once
    ...
    let mut iter = "a.b.c".rsplitn(2, '.'); //~ manual_split_once
    let r = iter.next().unwrap();
    let l = iter.next().unwrap();
