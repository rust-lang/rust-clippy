...
// above() and below() test cases where the lint must be conditional on MSRV.
// 1.53 is when arrays started implementing `IntoIterator`.
#[clippy::msrv = "1.53"]
fn above() {
    for a in vec![1, 2, 3] {
        //~^ useless_vec
...
fn below() {
    ...
    vec![1, 2, 3].into_iter().for_each(|_| {});
