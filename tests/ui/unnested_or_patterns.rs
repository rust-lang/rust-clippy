...
fn main() {
    ...
    if let box 0 | box 2 = Box::new(0) {} //~ unnested_or_patterns
    if let box ((0 | 1)) | box (2 | 3) | box 4 = Box::new(0) {}
    //~^ unnested_or_patterns
    const C0: Option<u8> = Some(1);
    if let Some(1) | C0 | Some(2) = None {} //~ unnested_or_patterns
    if let &mut 0 | &mut 2 = &mut 0 {} //~ unnested_or_patterns
    if let x @ 0 | x @ 2 = 0 {} //~ unnested_or_patterns
    ...
    //~^ unnested_or_patterns
    if let [0] | [1] = [0] {} //~ unnested_or_patterns
    if let [x, 0] | [x, 1] = [0, 1] {} //~ unnested_or_patterns
...
#[clippy::msrv = "1.53"]
fn msrv_1_53() {
    if let [1] | [53] = [0] {} //~ unnested_or_patterns
}
