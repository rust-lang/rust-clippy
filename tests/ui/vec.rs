...
fn main() {
    on_slice(&vec![]); //~ useless_vec
    on_slice(&[]);
    on_mut_slice(&mut vec![]); //~ useless_vec

    on_slice(&vec![1, 2]); //~ useless_vec
    on_slice(&[1, 2]);
    on_mut_slice(&mut vec![1, 2]); //~ useless_vec
    ...
    let _x: i32 = vec![1, 2, 3].iter().sum(); //~ useless_vec

    // Do lint
    let mut x = vec![1, 2, 3]; //~ useless_vec
    x.fill(123);
    dbg!(x[0]);
    dbg!(x.len());
    dbg!(x.iter().sum::<i32>());

    let _x: &[i32] = &vec![1, 2, 3]; //~ useless_vec
