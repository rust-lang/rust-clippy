...
fn main() {
    ...
    let bad_micros = dur.subsec_nanos() / 1_000; //~ duration_subsec
    let good_micros = dur.subsec_micros();
    assert_eq!(bad_micros, good_micros);

    // Handle refs
    let _ = (&dur).subsec_nanos() / 1_000; //~ duration_subsec

    // Handle constants
    const NANOS_IN_MICRO: u32 = 1_000;
    let _ = dur.subsec_nanos() / NANOS_IN_MICRO; //~ duration_subsec
