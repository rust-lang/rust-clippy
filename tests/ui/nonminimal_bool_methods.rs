...
fn methods_with_negation() {
    ...
    let _ = a.is_some();
    let _ = !a.is_some(); //~ nonminimal_bool
    let _ = a.is_none();
    let _ = !a.is_none(); //~ nonminimal_bool
    let _ = b.is_err();
    let _ = !b.is_err(); //~ nonminimal_bool
    let _ = b.is_ok();
    let _ = !b.is_ok(); //~ nonminimal_bool
    let c = false;
    let _ = !(a.is_some() && !c); //~ nonminimal_bool
    let _ = !(a.is_some() || !c); //~ nonminimal_bool
    let _ = !(!c ^ c) || !a.is_some(); //~ nonminimal_bool
    let _ = (!c ^ c) || !a.is_some(); //~ nonminimal_bool
    let _ = !c ^ c || !a.is_some(); //~ nonminimal_bool
}
...
fn warn_for_built_in_methods_with_negation() {
    ...
    if !res.is_err() {} //~ nonminimal_bool

    let res = Some(1);
    if !res.is_some() {} //~ nonminimal_bool
    if !res.is_none() {} //~ nonminimal_bool
}
