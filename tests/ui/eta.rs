...
fn main() {
    let a = Some(1u8).map(|a| foo(a)); //~ redundant_closure
    ...
    let e = Some(1u8).map(|a| generic(a)); //~ redundant_closure
...
fn passes_fn_mut(mut x: Box<dyn FnMut()>) {
    requires_fn_once(|| x()); //~ redundant_closure
}
...
fn test_redundant_closure_with_function_pointer() {
    ...
    let a = Some(1u8).map(|a| foo_ptr(a)); //~ redundant_closure
}

fn test_redundant_closure_with_another_closure() {
    let closure = |a| println!("{}", a);
    let a = Some(1u8).map(|a| closure(a)); //~ redundant_closure
}
...
fn mutable_closure_used_again(x: Vec<i32>, y: Vec<i32>, z: Vec<i32>) {
    ...
    x.into_iter().for_each(|x| add_to_res(x)); //~ redundant_closure
    y.into_iter().for_each(|x| add_to_res(x)); //~ redundant_closure
    z.into_iter().for_each(|x| add_to_res(x)); //~ redundant_closure
}
...
fn mutable_closure_in_loop() {
    ...
    for _ in 0..5 {
        Some(1).map(|n| closure(n)); //~ redundant_closure

        let mut value = 0;
        let mut in_loop = |n| value += n;
        Some(1).map(|n| in_loop(n)); //~ redundant_closure
    }
}
...
pub fn mutable_impl_fn_mut(mut f: impl FnMut(), mut f_used_once: impl FnMut()) -> impl FnMut() {
    ...
    takes_fn_mut(|| f()); //~ redundant_closure

    fn takes_fn_once(_: impl FnOnce()) {}
    takes_fn_once(|| f()); //~ redundant_closure

    f();

    move || takes_fn_mut(|| f_used_once()) //~ redundant_closure
}
...
fn _mixed_late_bound_and_early_bound_regions() {
    ...
    let _ = f(&0, |x, y| f2(x, y)); //~ redundant_closure
}
