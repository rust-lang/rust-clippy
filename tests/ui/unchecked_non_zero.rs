...
// --- `size_of::<T>()` and unsigned arithmetic ---

fn size_of_arg(data: &[u8]) {
    // A non-ZST has a size of at least 1, so these cannot panic.
    data.chunks(size_of::<u64>());
    data.chunks_exact(core::mem::size_of::<u64>());

    // A ZST makes the size provably `0`.
    data.chunks(size_of::<()>());
    //~^ unchecked_non_zero

    // `size_of::<u8>()` is 1, which is a valid chunk size.
    data.chunks(size_of::<u8>());
    // A cast could truncate, so it is not looked through and the base stays unproven.
    7u32.ilog(size_of::<u8>() as u32);
    //~^ unchecked_non_zero
}

fn size_of_generic<T>(data: &[u8]) {
    // `T` has no known layout here and may be a ZST, so this stays linted.
    data.chunks(size_of::<T>());
    //~^ unchecked_non_zero
}

fn unsigned_addition(data: &[u8], n: usize) {
    // `n` is unsigned, so the sum is at least the constant side.
    data.chunks(n + 4);
    data.chunks(4 + n);
    data.chunks(n + n + 1);

    // Neither side is known to be large enough.
    data.chunks(n + 0);
    //~^ unchecked_non_zero
    data.chunks(n + n);
    //~^ unchecked_non_zero
...
fn signed_addition(a: i32, b: i32) {
    // A signed sum can be anything, so `+` proves nothing here.
    7i32.ilog(a + b);
    //~^ unchecked_non_zero
    // ...and the same shape on an unsigned base is fine.
    7u32.ilog(a.unsigned_abs() + 2);
}
