...
fn main() {
    ...
    let _ = v.iter().find(|&x| *x < 0).is_none(); //~ search_is_some

    //~v search_is_some
    let _ = (0..1).find(|x| **y == *x).is_none(); // one dereference less
    let _ = (0..1).find(|x| *x == 0).is_none(); //~ search_is_some
    ...
    //~^ search_is_some
    let _ = "hello world".find(&s2).is_none(); //~ search_is_some
    let _ = "hello world".find(&s2[2..]).is_none();
    //~^ search_is_some
    // caller of `find()` is a `String`
    let _ = s1.find("world").is_none(); //~ search_is_some
    let _ = s1.find(&s2).is_none(); //~ search_is_some
    let _ = s1.find(&s2[2..]).is_none(); //~ search_is_some
    // caller of `find()` is slice of `String`
    let _ = s1[2..].find("world").is_none(); //~ search_is_some
    let _ = s1[2..].find(&s2).is_none(); //~ search_is_some
    let _ = s1[2..].find(&s2[2..]).is_none(); //~ search_is_some
}
