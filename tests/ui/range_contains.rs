...
fn main() {
    ...
    x >= 8 && x < 12; //~ manual_range_contains
    x < 42 && x >= 21; //~ manual_range_contains
    100 > x && 1 <= x; //~ manual_range_contains

    // also with inclusive ranges
    x >= 9 && x <= 99; //~ manual_range_contains
    x <= 33 && x >= 1; //~ manual_range_contains
    999 >= x && 1 <= x; //~ manual_range_contains
    ...
    // Fix #6315
    let y = 3.;
    y >= 0. && y < 1.; //~ manual_range_contains
    y < 0. || y > 1.; //~ manual_range_contains

    // handle negatives #8721
    x >= -10 && x <= 10; //~ manual_range_contains
    x >= 10 && x <= -10;
    y >= -3. && y <= 3.; //~ manual_range_contains
    y >= 3. && y <= -3.;
...
fn msrv_1_35() {
    let x = 5;
    x >= 8 && x < 35; //~ manual_range_contains
}
