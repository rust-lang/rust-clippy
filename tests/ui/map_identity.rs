...
fn main() {
    ...
    //~^ map_identity
    //~| map_identity
    let _: Option<u8> = Some(3).map(|x| x); //~ map_identity
    ...
    let _: Result<u32, u32> = Ok(1).map_err(|a| a); //~ map_identity
...
fn issue7189() {
    ...
    let _ = x.clone().map(|(x, y)| (x, y)); //~ map_identity

    //~v map_identity
    let _ = x.clone().map(|(x, y)| {
        return (x, y);
    });
    let _ = x.clone().map(|(x, y)| return (x, y)); //~ map_identity
...
fn issue11764() {
    ...
    let _ = x.iter().copied().map(|(x, y)| (x, y)); //~ map_identity
}
