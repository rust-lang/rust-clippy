...
use clippy_data_structures::{CountedIter, VecSet};
use core::ops::Range;
use core::{ptr, slice};
...
pub struct FieldData<'arena> {
    ...
    pub offset: u32,
    /// The projection data for this field.
...
pub struct PlaceData<'arena> {
    /// The offset and projection data for each immediate child.
    pub fields: &'arena IndexSlice<FieldIdx, FieldData<'arena>>,
    /// The number of values stored by this type, including all children.
    pub value_count: u32,
    /// Is a value stored for this type.
    pub has_value: bool,
}
    ...
    F: FnMut(Ty<'tcx>) -> bool,
{
    ...
    fn intern(&mut self, ty: Ty<'tcx>) -> &'arena PlaceData<'arena> {
        match self.projection_data.get(&ty) {
            Some(&data) => data,
            None => {
...
impl<'a> PlaceFilter<'a> {
    ...
    pub fn new_raw_borrow_filter(arena: &'a DroplessArena, body: &Body<'_>) -> Self {
        ...
        Self {
            filter: VecSet::from_sorted(v.borrows),
        }
    }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            if let Some(field) = self.current.fields.next() {
                ...
                if let Some(field) = self.parents.pop() {
                    self.current = field;
                } else {
                    let &(idx, projection) = self
                        .locals
                        .by_ref()
                        .filter(|&(_, data)| data.contains_values())
                        .next()?;
                    self.current = ResolvedParentsField {
...
impl<'arena> ResolvedPlace<'arena> for Resolved<'arena> {
    ...
    fn as_scalar_value(self) -> Option<Idx> {
        match self {
            Self::Value { data, idx, .. } => {
                debug_assert_eq!(data.value_count, data.has_value as u32);
