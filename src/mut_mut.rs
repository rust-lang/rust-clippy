...
impl LateLintPass for MutMut {
    ...
    fn check_ty(&mut self, cx: &LateContext, ty: &Ty) {
        unwrap_mut(ty).and_then(unwrap_mut).map_or((),
                                                   |_| {
                                                       span_lint(cx,
                                                                 MUT_MUT,
                                                                 ty.span,
                                                                 "generally you want to avoid \
                                                                  `&mut &mut _` if possible")
                                                   })
    }
}
                                     ...
                                     .unwrap_or_else(|| {
                                         if let TyRef(_, TypeAndMut{ty: _, mutbl: MutMutable}) =
                                             ...
                                             span_lint(cx,
                                                       MUT_MUT,
                                                       expr.span,
                                                       "this expression mutably borrows a mutable \
                                                        reference. Consider reborrowing")
                                         }
                                     })
                             })
}
