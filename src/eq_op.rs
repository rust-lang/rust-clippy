...
fn is_path_equal(left: &Path, right: &Path) -> bool {
    ...
    over(&left.segments, &right.segments, |l, r| {
...
fn both<X, F>(l: &Option<X>, r: &Option<X>, mut eq_fn: F) -> bool
    where F: FnMut(&X, &X) -> bool
{
    l.as_ref().map_or_else(|| r.is_none(),
                           |x| r.as_ref().map_or(false, |y| eq_fn(x, y)))
}
...
fn is_cast_ty_equal(left: &Ty, right: &Ty) -> bool {
    match (&left.node, &right.node) {
        ...
        (&TyPtr(ref lmut), &TyPtr(ref rmut)) =>
            lmut.mutbl == rmut.mutbl && is_cast_ty_equal(&*lmut.ty, &*rmut.ty),
