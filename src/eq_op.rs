...
pub fn is_exp_equal(cx: &LateContext, left: &Expr, right: &Expr) -> bool {
    ...
    match (&left.node, &right.node) {
        ...
        (&ExprCast(ref lx, ref lt), &ExprCast(ref rx, ref rt)) => is_exp_equal(cx, lx, rx) &&
                                                                  is_cast_ty_equal(lt, rt),
        _ => false,
    }
}
...
fn is_path_equal(left: &Path, right: &Path) -> bool {
    ...
    over(&left.segments,
         &right.segments,
         |l, r| {
             l.identifier.name == r.identifier.name && l.identifier.ctxt == r.identifier.ctxt &&
...
fn is_cast_ty_equal(left: &Ty, right: &Ty) -> bool {
    match (&left.node, &right.node) {
        ...
        (&TyPtr(ref lmut), &TyPtr(ref rmut)) => lmut.mutbl == rmut.mutbl &&
                                                is_cast_ty_equal(&*lmut.ty, &*rmut.ty),
        (&TyRptr(_, ref lrmut), &TyRptr(_, ref rrmut)) => lrmut.mutbl == rrmut.mutbl &&
                                                          is_cast_ty_equal(&*lrmut.ty, &*rrmut.ty),
        (&TyPath(ref lq, ref lpath), &TyPath(ref rq, ref rpath)) => both(lq, rq, is_qself_equal) &&
