...
fn check_compare(cx: &LateContext, bit_op: &Expr, cmp_op: BinOp_, cmp_value: u64, span: &Span) {
    if let ExprBinary(ref op, ref left, ref right) = bit_op.node {
        ...
        fetch_int_literal(cx, right)
            ...
            .map_or((),
                    |mask| check_bit_mask(cx, op.node, cmp_op, mask, cmp_value, span))
    }
}
                    ...
                    if mask_value & cmp_value != cmp_value {
                        if cmp_value != 0 {
                            span_lint(cx,
                                      BAD_BIT_MASK,
                                      *span,
                                      &format!("incompatible bit mask: `_ & {}` can never be \
                                                equal to `{}`",
                                               mask_value,
                                               cmp_value));
                        }
                ...
                BiBitOr => {
                    if mask_value | cmp_value != cmp_value {
                        span_lint(cx,
                                  BAD_BIT_MASK,
                                  *span,
                                  &format!("incompatible bit mask: `_ | {}` can never be equal \
                                            to `{}`",
                                           mask_value,
                                           cmp_value));
                    }
                ...
                BiBitAnd => {
                    if mask_value < cmp_value {
                        span_lint(cx,
                                  BAD_BIT_MASK,
                                  *span,
                                  &format!("incompatible bit mask: `_ & {}` will always be \
                                            lower than `{}`",
                                           mask_value,
                                           cmp_value));
                    } else {
                ...
                BiBitOr => {
                    if mask_value >= cmp_value {
                        span_lint(cx,
                                  BAD_BIT_MASK,
                                  *span,
                                  &format!("incompatible bit mask: `_ | {}` will never be lower \
                                            than `{}`",
                                           mask_value,
                                           cmp_value));
                    } else {
                ...
                BiBitAnd => {
                    if mask_value <= cmp_value {
                        span_lint(cx,
                                  BAD_BIT_MASK,
                                  *span,
                                  &format!("incompatible bit mask: `_ & {}` will never be \
                                            higher than `{}`",
                                           mask_value,
                                           cmp_value));
                    } else {
...
fn check_ineffective_lt(cx: &LateContext, span: Span, m: u64, c: u64, op: &str) {
    if c.is_power_of_two() && m < c {
        span_lint(cx,
                  ...
                  span,
                  &format!("ineffective bit mask: `x {} {}` compared to `{}`, is the same as x \
                            compared directly",
                           op,
                           m,
                           c));
