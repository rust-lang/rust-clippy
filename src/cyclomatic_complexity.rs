    ...
    fn check<'a, 'tcx>(&mut self, cx: &'a LateContext<'a, 'tcx>, block: &Block, span: Span) {
        ...
        if cc + divergence < narms {
            ...
            if rust_cc > self.limit.limit() {
                cx.span_lint_help(CYCLOMATIC_COMPLEXITY,
                                  span,
                                  &format!("The function has a cyclomatic complexity of {}.",
                                           rust_cc),
...
fn report_cc_bug(cx: &LateContext, cc: u64, narms: u64, div: u64, span: Span) {
    cx.sess().span_bug(span,
                       &format!("Clippy encountered a bug calculating cyclomatic complexity: cc \
...
fn report_cc_bug(cx: &LateContext, cc: u64, narms: u64, div: u64, span: Span) {
    if cx.current_level(CYCLOMATIC_COMPLEXITY) != Level::Allow {
        cx.sess().span_note(span,
                            &format!("Clippy encountered a bug calculating cyclomatic \
                                      complexity (hide this message with \
                                      `#[allow(cyclomatic_complexity)]`): cc = {}, arms = {}, \
