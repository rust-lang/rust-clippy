...
fn check_cmp(cx: &LateContext, span: Span, left: &Expr, right: &Expr, op: &str) {
    ...
    match (&left.node, &right.node) {
        (&ExprLit(ref lit), &ExprMethodCall(ref method, _, ref args)) => check_len_zero(cx,
                                                                                        span,
                                                                                        method,
                                                                                        args,
                                                                                        lit,
                                                                                        op),
        (&ExprMethodCall(ref method, _, ref args), &ExprLit(ref lit)) => check_len_zero(cx,
                                                                                        span,
                                                                                        method,
                                                                                        args,
                                                                                        lit,
...
fn check_len_zero(cx: &LateContext,
                  span: Span,
                  method: &SpannedIdent,
                  args: &[P<Expr>],
                  lit: &Lit,
    ...
    if let Spanned{node: LitInt(0, _), ..} = *lit {
        if method.node.name == "len" && args.len() == 1 && has_is_empty(cx, &args[0]) {
            span_lint(cx,
                      LEN_ZERO,
                      span,
                      &format!(
    ...
    fn is_is_empty(cx: &LateContext, id: &ImplOrTraitItemId) -> bool {
        if let &MethodTraitItemId(def_id) = id {
            if let ty::MethodTraitItem(ref method) = cx.tcx.impl_or_trait_item(def_id) {
                method.name == "is_empty" && method.fty.sig.skip_binder().inputs.len() == 1
            } else {
                false
            }
        } else {
        ...
        cx.tcx
          ...
          .map_or(false,
                  |ids| {
                      ids.iter().any(|iid| {
                          impl_items.get(iid)
                                    .map_or(false, |iids| iids.iter().any(|i| is_is_empty(cx, i)))
                      })
                  })
    }
