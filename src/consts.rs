...
impl PartialEq for Constant {
    fn eq(&self, other: &Constant) -> bool {
        match (self, other) {
            (&ConstantStr(ref ls, ref lsty),
             &ConstantStr(ref rs, ref rsty)) => ls == rs && lsty == rsty,
            ...
            (&ConstantRepeat(ref lv, ref ls),
             &ConstantRepeat(ref rv, ref rs)) => ls == rs && lv == rv,
...
fn format_byte(fmt: &mut fmt::Formatter, b: u8) -> fmt::Result {
    if b == b'\\' {
        ...
        write!(fmt,
               "{}",
               char::from_u32(b as u32).expect("all u8 are valid char"))
    } else if b == 0x0a {
        write!(fmt, "\\n")
    } else if b == 0x0d {
        write!(fmt, "\\r")
        ...
        match *self {
            ...
            ConstantInt(ref i, ref ity) => {
                let (sign, suffix) = match *ity {
                    ...
                    LitIntType::UnsignedIntLit(ref uity) => ("", uint_ty_to_string(*uity)),
                    ...
                    match (l, r) {
                        ...
                        (ConstantInt(l64, lty), ConstantInt(r64, rty)) => {
                            match (is_negative(lty), is_negative(rty)) {
                                ...
                                (true, false) => {
                                    unify_int_type(lty, rty, Minus).and_then(|ty| {
                                        l64.checked_add(r64).map(|v| ConstantInt(v, ty))
                                    })
                                }
                                (false, true) => {
                                    unify_int_type(lty, rty, Plus).and_then(|ty| {
                                        l64.checked_add(r64).map(|v| ConstantInt(v, ty))
                                    })
                                }
                            }
