...
impl PartialEq for Constant {
    fn eq(&self, other: &Constant) -> bool {
        match (self, other) {
            (&ConstantStr(ref ls, ref lsty), &ConstantStr(ref rs, ref rsty)) => ls == rs &&
                                                                                lsty == rsty,
            ...
            (&ConstantRepeat(ref lv, ref ls), &ConstantRepeat(ref rv, ref rs)) => ls == rs &&
                                                                                  lv == rv,
...
impl PartialOrd for Constant {
    fn partial_cmp(&self, other: &Constant) -> Option<Ordering> {
        match (self, other) {
            (&ConstantStr(ref ls, ref lsty), &ConstantStr(ref rs, ref rsty)) => if lsty != rsty {
                None
            } else {
                Some(ls.cmp(rs))
            },
...
fn constant_negate(o: Constant) -> Option<Constant> {
    Some(match o {
        ConstantInt(value, ty) => ConstantInt(value,
                                              match ty {
...
fn unify_int_type(l: LitIntType, r: LitIntType, s: Sign) -> Option<LitIntType> {
    match (l, r) {
        ...
        (UnsignedIntLit(lty), UnsignedIntLit(rty)) => if s == Plus && lty == rty {
...
impl<'c, 'cc> ConstEvalLateContext<'c, 'cc> {
        ...
        match e.node {
            ...
            ExprRepeat(ref value, ref number) =>
                self.binop_apply(value,
                             number,
                             |v, n| Some(ConstantRepeat(Box::new(v), n.as_u64() as usize))),
            ExprUnary(op, ref operand) => self.expr(operand).and_then(|o| {
                match op {
    ...
    fn fetch_path(&mut self, e: &Expr) -> Option<Constant> {
        if let Some(lcx) = self.lcx {
            ...
            if let Some(&PathResolution { base_def: DefConst(id), ..}) = lcx.tcx
                                                                            .def_map
                                                                            .borrow()
                                                                            .get(&e.id) {
                maybe_id = Some(id);
    ...
    fn ifthenelse(&mut self,
                  cond: &Expr,
                  then: &Block,
                  otherwise: &Option<P<Expr>>)
                  -> Option<Constant> {
                                      ...
                                      |l, r| {
                                          match (l, r) {
                                              ...
                                              (ConstantInt(l64, lty), ConstantInt(r64, rty)) =>
                                                  ...
                                                  (false, false) => sub_int(l64,
                                                                            lty,
                                                                            r64,
                                                                            rty,
                                                                            r64 > l64),
                                                  (true, true) => sub_int(l64,
                                                                          lty,
                                                                          r64,
                                                                          rty,
                                                                          l64 > r64),
