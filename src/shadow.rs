        ...
        if is_self_shadow(name, expr) {
            ...
            if contains_self(name, expr) {
                span_note_and_lint(cx,
                                   SHADOW_REUSE,
                                   lspan,
                                   &format!("{} is shadowed by {} which reuses the original \
                                             value",
                                            snippet(cx, lspan, "_"),
                                            snippet(cx, expr.span, "..")),
                                   expr.span,
...
fn contains_self(name: Name, expr: &Expr) -> bool {
    match expr.node {
        ...
        ExprCall(ref fun, ref args) =>
            contains_self(name, fun) || args.iter().any(|ref a| contains_self(name, a)),
        // special ones
        ExprIf(ref cond, ref then, ref otherwise) =>
        ...
        ExprMatch(ref e, ref arms, _) => contains_self(name, e) ||
...
fn contains_pat_self(name: Name, pat: &Pat) -> bool {
    match pat.node {
        ...
        PatEnum(_, ref opats) =>
            opats.as_ref().map_or(false,
                                  |pats| pats.iter().any(|p| contains_pat_self(name, p))),
        PatQPath(_, ref path) => path_eq_name(name, path),
        PatStruct(_, ref fieldpats, _) =>
            fieldpats.iter().any(|ref fp| contains_pat_self(name, &fp.node.pat)),
