        ...
        if let ExprLoop(ref block, _) = expr.node {
            ...
            if block.stmts.is_empty() && block.expr.is_none() {
                span_lint(cx,
                          EMPTY_LOOP,
                          expr.span,
                          "empty `loop {}` detected. You may want to either use `panic!()` or \
                           add `std::thread::sleep(..);` to the loop body.");
            }
                        ...
                        if visitor.nonindex {
                            span_lint(cx,
                                      ...
                                      &format!("the loop variable `{}` is used to index `{}`. \
                                                ...
                                                {}.iter().enumerate()` or similar iterators",
                                               ident.node.name,
                                               indexed,
                                               ident.node.name,
                                               indexed));
            ...
            if let Some(stop_idx @ Constant::ConstantInt(..)) = constant_simple(stop_expr) {
                ...
                if start_idx > stop_idx {
                    span_help_and_lint(cx,
                                       ...
                                       &format!("Consider using `({}..{}).rev()` if you are \
                                                 attempting to iterate over this range in \
                                                 reverse",
                                                stop_idx,
                                                start_idx));
            ...
            if method_name.as_str() == "iter" || method_name.as_str() == "iter_mut" {
               ...
               match_trait_method(cx, arg, &["core", "iter", "Iterator"]) {
                span_lint(cx,
                          ITER_NEXT_LOOP,
                          expr.span,
                          "you are iterating over `Iterator::next()` which is an Option; this \
                           will compile but is probably not what you want");
            }
        }
    }
                ...
                if visitor2.state == VarState::Warn {
                    if let Some(name) = visitor2.name {
                        span_lint(cx,
                                  EXPLICIT_COUNTER_LOOP,
                                  expr.span,
                                  &format!("the variable `{0}` is used as a loop counter. \
                                            Consider using `for ({0}, item) in {1}.enumerate()` \
