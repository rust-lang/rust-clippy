                            ...
                            if let Some(error_type) = get_error_type(cx, result_type) {
                                if has_debug_impl(error_type, cx) {
                                    span_lint(cx,
                                              OK_EXPECT,
                                              expr.span,
                                              "called `ok().expect()` on a Result value. You can \
                    ...
                    if let ExprMethodCall(ref inner_name, _, ref inner_args) = args[0].node {
                        if inner_name.node.as_str() == "map" &&
                           match_type(cx, cx.tcx.expr_ty(&inner_args[0]), &OPTION_PATH) {
                            // lint message
                            let msg = "called `map(f).unwrap_or(a)` on an Option value. This can \
                                       be done more directly by calling `map_or(a, f)` instead";
                ...
                "unwrap_or_else" => {
                    if let ExprMethodCall(ref inner_name, _, ref inner_args) = args[0].node {
                        if inner_name.node.as_str() == "map" &&
                           match_type(cx, cx.tcx.expr_ty(&inner_args[0]), &OPTION_PATH) {
                            // lint message
                            let msg = "called `map(f).unwrap_or_else(g)` on an Option value. This \
                                       can be done more directly by calling `map_or_else(g, f)` \
                                       instead";
                            ...
                            if same_span && !multiline {
                                span_note_and_lint(cx,
                                                   ...
                                                   msg,
                                                   expr.span,
                                                   &format!("replace this with map_or_else({1}, \
                                                             {0})",
                                                            map_arg,
                                                            unwrap_arg));
                            } else if same_span && multiline {
...
impl SelfKind {
    ...
    fn matches_explicit_type(&self, ty: &Ty, allow_value_for_ref: bool) -> bool {
        match (self, &ty.node) {
            ...
            (&RefSelf,
             &TyRptr(_, MutTy { mutbl: Mutability::MutImmutable, .. })) => true,
            (&RefMutSelf,
             &TyRptr(_, MutTy { mutbl: Mutability::MutMutable, .. })) => true,
